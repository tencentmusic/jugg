...
2) **常见任务**：若是标准场景（新增编译器、增量资源调试、命令行/CI、RPC 扩展、远程编译、部署排查），先看 `07_cookbook.md`。
3) **定位专题**：按模块需求跳到对应文档：
...
4) **不确定场景**：先看 `99_index.md` 查关键词/模块，再决定是否展开某一文档的相关小节。
5) **判断是否已有接口/避免造轮子**：
   - 先查 `07_cookbook.md` 是否已有现成任务/步骤。
   - 再查 `98_code_map.md` 的对应模块/类，确认已有入口（状态/日期）。
...
- 有明确任务 → 先看 `07_cookbook.md`，若需细节再钻入单一专题文档。
- 仅需架构/全貌 → 读 `00_overview.md` 和 `01_architecture.md`，不要展开其它文档。
- 不要：把全部 02/03/04/05 文档一次性放入上下文。
...
- 针对任务：
  - “如果是增量编译或部署问题，先查 cookbook 的对应条目，再决定是否打开 compile/deploy 文档。”
...
- [ ] 高频任务/扩展点变动，已更新 `07_cookbook.md` 示例指向或步骤。
- [ ] 相关专题文档（02/03/04/05 系列）是否需补充“扩展点/用法”小节，已评估并处理。
