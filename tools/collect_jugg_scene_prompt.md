...
采集结束后会自动打开结果文件夹，并选中 `jugg_scene_*.zip`。把这个 zip 发回维护者即可，不要在对话里粘贴二进制内容。
...
- 为了“方便”省略设备 overlay、APK、R.jar 或 log 采集

这些操作会覆盖现场。手机保持解锁，USB 调试保持开启，采集期间不要操作 App。

沙箱拦文件、adb 或 USB 设备时，申请本机完整权限后重试，不要改成跳过 adb。

### 1. 定位工程

1. 当前工作区根目录若存在 `build/jugg/`，就用它。
2. 否则在工作区内 `find . -maxdepth 4 -type d -path '*/build/jugg'`；只命中一个就用其父目录作为工程根。
3. 找不到就停下来问用户工程路径，不要编造目录，不要继续采集。
4. 已知应用包名时带 `--package-name`；多设备且用户指定了设备时带 `--device-serial`。不要猜错包名；不确定就让脚本自动推断。

### 2. 落盘脚本并执行

把下面「采集脚本」原样写入 `/tmp/collect_jugg_scene.sh`，不要改逻辑。然后：

```bash
chmod +x /tmp/collect_jugg_scene.sh
bash /tmp/collect_jugg_scene.sh "<ANDROID_PROJECT_ROOT>"
```

默认输出到 `~/Desktop`。执行完脚本自己会打包 zip 并在 Finder 里选中它。
...
usage() {
  cat <<'USAGE'
Usage:
  collect_jugg_scene.sh [project_dir] [--output-root DIR] [--include-apks yes|no] [--package-name NAME] [--device-serial SERIAL] [--skip-adb] [--no-open]
...
package_zip() {
  ...
  if [[ -n "$ZIP_PATH" && -f "$ZIP_PATH" ]]; then
    perl -pi -e "s#^Zip: pending#Zip: $ZIP_PATH#" "$OUT_DIR/summary.txt" 2>/dev/null || true
  else
    ZIP_PATH=""
    perl -pi -e "s#^Zip: pending#Zip: unavailable#" "$OUT_DIR/summary.txt" 2>/dev/null || true
  fi
}
...
open_result() {
  ...
  if command -v open >/dev/null 2>&1; then
    if [[ -n "$ZIP_PATH" && -f "$ZIP_PATH" ]]; then
      open -R "$ZIP_PATH" || open "$OUT_DIR" || true
    else
      open "$OUT_DIR" || true
    fi
