...
class RemoteClient(project: Project, parent: Disposable) : IRemoteClient, Disposable {
    ...
    var terminalOutputListener: TerminalOutputListener? = null
    ...
    private fun invoke(channel: Channel, command: ISshCommand): Int {
        printToStream("${command::class.simpleName} exec start")
        ...
        while (true) {
            ...
            if (channel.isClosed) {
                printToStream("exit-status: " + channel.exitStatus)
                result = RESULT_CHANNEL_CLOSED
                break
            }
        }

        printToStream("${command::class.simpleName} exec finished with result: $result")
        return result
    }
    ...
    private fun printToStreamError(line: String, e: Exception? = null) {
        ...
        terminalOutputListener?.onOutput(line)
        e?.let {
            terminalOutputListener?.onOutput(e.toString())
        }
    }
