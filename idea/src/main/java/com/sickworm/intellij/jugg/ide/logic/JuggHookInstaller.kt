...
/**
 * Installs Claude hook config that enforces Android-change verification via
...
object JuggHookInstaller {
    ...
    /**
     * Installs hook config into ~/.claude/settings.json and ~/.claude-internal/settings.json.
     */
    fun installForClaude(
        ...
        val results = targets.map { target ->
            runCatching { installTarget(target) }
                .onFailure { error ->
                    logger.warn("[Install Claude Hooks] failed path=${target.file.path}", error)
                }
                .getOrElse { error ->
                    HookInstallResult(
    ...
    /**
     * JSON adapter for Claude settings. It only touches SessionStart/Stop entries
     * required by Jugg verification policy.
     */
    private class ClaudeSettingsJsonHookConfigAdapter(
        ...
        override fun merge(existingContent: String?): HookMergeResult {
            ...
            val sessionStartHooks = hooks.ensureArray("SessionStart").also { changed = changed || it.second }.first
            val sessionStartEntry = findOrCreateSessionStartEntry(sessionStartHooks)
                .also { changed = changed || it.second }
                .first
            val sessionStartHookArray = sessionStartEntry.ensureArray("hooks").also { changed = changed || it.second }.first
            val sessionStartHookAdded = ensureCommandHook(sessionStartHookArray, startCommand)
            changed = changed || sessionStartHookAdded

            val stopHooks = hooks.ensureArray("Stop").also { changed = changed || it.second }.first
            val entry = findOrCreateMatcherEntry(stopHooks).also { changed = changed || it.second }.first
            val hookArray = entry.ensureArray("hooks").also { changed = changed || it.second }.first
            val stopHookAdded = ensureCommandHook(hookArray, stopCommand)
            changed = changed || stopHookAdded
        ...
        private fun findOrCreateSessionStartEntry(sessionStartHooks: JsonArray): Pair<JsonObject, Boolean> {
            sessionStartHooks.forEach { element ->
                if (!element.isJsonObject) return@forEach
                val obj = element.asJsonObject
                val matcher = obj.get("matcher")
                if (matcher == null || matcher.isJsonNull || matcher.asString == MATCHER_ALL) {
                    return obj to false
                }
            }
            ...
            sessionStartHooks.add(newEntry)
            return newEntry to true
        }

        private fun findOrCreateMatcherEntry(stopHooks: JsonArray): Pair<JsonObject, Boolean> {
            stopHooks.forEach { element ->
                if (!element.isJsonObject) return@forEach
                val obj = element.asJsonObject
                if (obj.get("matcher")?.asString == MATCHER_ALL) {
                    return obj to false
                }
            }
            val newEntry = JsonObject().apply {
                addProperty("matcher", MATCHER_ALL)
                add("hooks", JsonArray())
            }
            stopHooks.add(newEntry)
            return newEntry to true
        }

        private fun ensureCommandHook(hookArray: JsonArray, command: String): Boolean {
...
data class HookInstallSummary(
    ...
    val isAllSuccess: Boolean
        get() = results.isNotEmpty() && results.all { it.status == "ok" || it.status == "skip" }
