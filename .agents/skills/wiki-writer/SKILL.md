...
同一页面同时满足以下条件时，优先拆分而不是用一个弱主线拼接：

- 包含两个或更多独立的读者问题。
- 行为 owner、失败模式或回退条件彼此独立。
- 任一主题都能形成完整的“问题、机制、边界”论证。

已有路由承担外部入口时，保留简短分流页或兼容入口，不要直接删除路径。

## 写前确定页面主线

编辑前用一至三句话或几个要点明确页面主线，仅供写作过程使用，不要直接复制到正文。至少回答：
...
核心 concept 页面还要判断普通 Android 开发者是否理解目标机制的输入、关键阶段和产物。读者缺少这层基础时，先用最少篇幅说明标准工作模式，再解释 Jugg 复用、替代或补充了哪些环节。领域介绍只需支撑后续论证，不扩写成通用框架教程。

读者已经熟悉基础机制，且页面围绕真实失败展开时，再判断：

- 常规或局部处理留下了什么具体旧状态或失败。
- Jugg 通过什么机制处理这项影响。
- 缺少这套机制时会出现什么用户可见结果。
- 哪些条件下这套机制不适用或应回退。

这些内容无法由当前文档、代码、日志或稳定复现支撑时，不要继续扩大结论。

## 选择概念页叙事起点

根据读者的前置知识选择叙事起点，不固定开场和章节顺序：

- 读者熟悉基础机制，且存在真实局部方案或可观察故障时，可以从具体修改、遗漏状态和用户可见结果切入。
- 读者不熟悉基础机制时，先说明标准输入、阶段和产物，再说明 Jugg 改变了哪些环节、状态如何流转以及何时回退。

这两种只是论证起点，不是文章模板。比较表和完整流程只在存在真实映射或多个依赖阶段时使用。

标题要直接描述内容。不要用“背景”“痛点”“核心解法”“方案价值”“边界与代价”等元结构标题代替具体问题。
...
1. 检查所有新增相对链接能够解析到现有源码页面。
2. 执行 `git diff --check`。
3. 在 `docs/wiki` 下执行不包含 dev-only 页面配置的 `npm run build`。
4. 确认 `docs/wiki/.vitepress/dist/` 下生成预期 HTML，并包含新标题或能够区分本次改动的章节。
5. 复查最终 diff，排除无证据的行为主张、过时历史状态、concept/capability 重复和意外暴露的源码细节。
6. 按仓库 commit 规范，只提交本次任务修改的文件。
