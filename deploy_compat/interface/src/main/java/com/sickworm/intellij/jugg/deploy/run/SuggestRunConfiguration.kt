...
data class SuggestRunConfiguration(
    ...
    companion object {
        ...
        /** Converts Gradle project and build identities into an exact task module path. */
        fun resolveGradleModulePath(
            gradleProjectPath: String,
            gradleBuildRoot: String,
            projectDir: File,
            externalProjectId: String?,
        ): String? {
            val modulePath = gradleProjectPath.trim(':').takeIf { it.isNotEmpty() } ?: return null
            val normalizedModulePath = ":$modulePath"
            if (File(gradleBuildRoot).absoluteFile.normalize() == projectDir.absoluteFile.normalize()) {
            ...
            val buildName = externalProjectId?.substringBefore(':')?.takeIf { it.isNotEmpty() }
                ?: File(gradleBuildRoot).name.takeIf { it.isNotEmpty() }
                ?: return null
            return ":$buildName$normalizedModulePath"
        }

        fun createCompileCommand(gradleModulePath: String, taskName: String): String {
            return "./gradlew :${gradleModulePath.trim(':')}:$taskName"
        }
