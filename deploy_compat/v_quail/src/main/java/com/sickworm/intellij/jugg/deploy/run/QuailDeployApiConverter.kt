...
class QuailDeviceAdapter(
    ...
    override fun equals(other: Any?): Boolean = other is QuailDeviceAdapter && studioDevice == other.studioDevice

    override fun hashCode(): Int = studioDevice.hashCode()
}

/** Converts deployment values at the Quail Android Studio API boundary. */
class QuailDeployApiConverter {
    private val devices = WeakHashMap<StudioDevice, WeakReference<QuailDeviceAdapter>>()
    ...
    fun toJuggDevice(device: StudioDevice): IDevice = synchronized(devices) {
        devices[device]?.get() ?: QuailDeviceAdapter(device).also { devices[device] = WeakReference(it) }
    }
    ...
    private fun StudioApk.toJuggApk(entries: Map<String, ApkEntry>): Apk {
        return Apk(name, checksum, path, packageName, libraryAbis, targetPackages, sdkLibraries, entries, this)
    }
}
