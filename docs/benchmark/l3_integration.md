...
**级别**：L3
**工具**：activity_stack → figma_layout_verify → eval_view
...
**期望调用序列**：
1. `ui_find(target={resourceId: "view_mcp_bg_block"})` 或 通过文本/位置推断
2. `eval_view(target={resourceId: "view_mcp_bg_block"}, expressions=["getWidth()", "getHeight()", "getBackground()"])`
...
2. `eval_view(target={resourceId: "tv_mcp_label"}, expressions=["getPaddingLeft()"])`
3. LLM 对比设计预期，给出验证结论
...
**级别**：L3
**工具**：figma_layout_verify → eval_view
...
**级别**：L3
**工具**：restart_app → activity_stack → figma_layout_verify
...
**期望调用序列**：
1. `eval_view(target={resourceId: "tv_mcp_style_title"}, expressions=["getCurrentTextColor()", "getTextSize()"])`
2. `eval_view(target={resourceId: "view_mcp_bg_block"}, expressions=["getBackground()", "getWidth()", "getHeight()"])`
3. `eval_view(target={resourceId: "tv_mcp_body_text"}, expressions=["getCurrentTextColor()", "getTextSize()"])`
...
**期望调用序列**：
1. `ui_find(target={text: "Unique MCP Target"})` — 定位按钮
2. `tap` — 点击按钮
3. `eval_view(target={resourceId: "tv_mcp_action_state"}, expressions=["getText().toString()"])` — 读取状态
...
**期望调用序列**：
1. `eval_view(target={resourceId: "ll_mcp_parent"}, expressions=["isShown()"])`
2. `eval_view(target={resourceId: "iv_mcp_icon"}, expressions=["isShown()"])`
3. `eval_view(target={resourceId: "tv_mcp_label"}, expressions=["isShown()"])`
...
**级别**：L3
**工具**：ui_find → figma_layout_verify
...
**期望调用序列**：
1. `eval_view(target={resourceId: "tv_mcp_style_title"}, expressions=["getTextSize()", "getCurrentTextColor()"])`
2. `eval_view(target={resourceId: "tv_mcp_body_text"}, expressions=["getTextSize()", "getCurrentTextColor()"])`
3. 对比报告
...
**期望调用序列**：
1. `ui_find` — 查找所有 Button
2. 对每个找到的 Button 调用 `eval_view(expressions=["isClickable()"])`
...
### TC-I14：完整页面健康检查（综合多工具）

**级别**：L3
**工具**：activity_stack → figma_layout_verify → eval_view（多次）
...
**级别**：L3
**工具**：compile_and_deploy → activity_stack → figma_layout_verify
