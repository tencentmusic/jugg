...
| control panel snapshot | `JuggControlPanelModel` | `JuggControlPanelController` 项目级持有；保存待处理文件、当前阶段、原始 compile/deploy 事实、用户业务动作、会话成功统计、有界 Recent Runs 与最近 200 条核心事件；MCP、Sync、App、User Action 事件只进入事件历史，不覆盖运行任务 |
...
Sync 完成或被 IDE 标记为 `SKIPPED` 后，`tryCreateRunConfigurations()` 会读取 Android Studio 当前 Active Build Variant 对应的 Gradle command，并按需创建对应 Jugg Configuration。项目原来没有可用 Jugg Configuration 时，首次创建后自动选择；已有 Jugg Configuration 且当前 selected Configuration 不是 Jugg 时只创建、不改变选择。当前 selected Configuration 是 Jugg 时，只有 source command 和 active suggestion 都精确符合单 task `./gradlew :modulePath:assemble{Variant}`、两边实际 Gradle module path 相同、variant 明确不同，并且能唯一找到同时精确匹配 suggestion command 与 APK output path 的目标配置时，才自动切换。module path 与 variant 直接从 command 解析，不从 Jugg module identity 反向拼装，因此 `:zxphone5.0` 这类 path segment 内的点号不会被误判为层级。`deployDebug` / `packageDebug` / `uploadDebug`、`happyBuild`、带 `--offline` / `-Pxxx` 的 assemble、多 task 或脚本包装等自定义 command 全部保持用户选择；允许边界漏切，不通过 task 名后缀猜测用户意图。模块首个 Configuration 沿用 `jugg:<module>`，该名称已存在时使用 `jugg:<module>:<variant>`；创建和去重仍沿用 suggestion 的唯一 task 逻辑，但宽松 task 匹配不再用于自动选择目标。suggestion 无法解析出唯一标准 assemble task 时禁止自动切换当前 Configuration。`FullBuildInfo.compileCommand` 不参与 Sync 阶段的 Configuration 创建和自动切换，仅用于 CLI/MCP/RPC 运行时的历史配置兜底，以及切换后首次 Run 的基线判断；command 不一致时，`JuggCompileHelper.preprocessIncrementalCompile()` 会强制走 Gradle full build，成功后刷新基线。日志会同时打印 `last=` 与 `current=` 两条 command。
...
Jugg module identity 与 Gradle module path 分开保存语义：前者用于 Configuration 命名和同模块匹配，后者直接使用 `GradleProjectPath` 的原始 project path 生成 command。root build 会移除根项目名前缀，例如 `Root.app -> app`，但 task path 保持 `:app:assembleDebug`；path segment 内的点号原样保留，例如 `:zxphone5.0:assembleDebug`。included build 保留 build identity，例如 `SMCommon.app -> SMCommon.app`，task path 为 `:SMCommon:app:assembleDebug`。`GradleProjectPath` 不可用时才回退 IDE module name 的 legacy 转换，不猜测点号来自模块层级还是模块名称。创建 Configuration 时还会按 suggestion 提供的唯一 Gradle task 做批内去重，避免附加参数差异或多个 Android Run Configuration 指向同一模块时生成 `(1)` 重复项。
...
- 默认 Run 配置由 `JuggManager.tryCreateRunConfigurations()` 通过 `AsDeployerCompat.getSuggestRunConfigurations()` 推断；配置名包含 variant，APK 路径使用 IDE model 的实际 build folder。Sync 后如果没有可用配置会短暂重试，并只在 source、variant、唯一标准目标三项均可确定时自动切换。
- Run Configuration 的 `More options` 只负责保存配置并打开 `Jugg Running Panel` Settings；稳定桥接接口仍保留返回空 ActionGroup 的兼容方法，不再创建旧下拉菜单。
- `/check_update` 不依赖完整编译或 deploy history。服务器选择由该请求触发：当前 URL 为空时先等待强制选服完成，已有 URL 时继续用它发起本次请求并异步刷新后续请求使用的服务器；构造期不再重复选服。收到配置后，`CheckUpdateHandler` 先保存既有默认自定义配置，再由 `ProjectDefaultSettingsApplier` 比较该工程 `PropertiesComponent` 中的 `jugg.defaultSettingsVersion`。只有版本与本地不同且版本、设置 Map 均非 null，应用器才一次性反射写入 `JuggSettings` 的同名公开可写基础类型属性；未知 key 和错误类型逐项跳过，每个成功或失败的属性均打印 debug 日志，设置应用完成后才记录版本。同一版本不覆盖用户修改，版本回滚也重新应用；setter 异常不记录版本。实际设置仍存储在全局 `PropertiesComponent`，多个工程不能长期持有互不影响的设置值。本次仅提供默认值下发，不改业务开关消费逻辑；成功后刷新 Control Panel Settings。
- `Jugg Running Panel` 的稳定层只创建 `JuggControlPanelHost`；Host 经 `IJuggManagerCaller.getJuggControlPanel(page): JComponent` 挂载当前 Jugg ClassLoader 创建的真实 Panel。Model、Snapshot、Event、Controller 和具体 Panel 类型都不进入 `ide_entry` 桥接接口，后续字段与 UI 变更可通过新 ClassLoader 生效。Host 初始为 initializing 占位态；由 Controller 统一收口与 Host 的显式生命周期协同：Manager 启动就绪时由 `JuggManager.init()` 调用 `controlPanelController.onManagerReady()` 刷新 Host 挂载真实面板，避免恢复窗口布局时因初始化时序卡在占位页；Manager 释放时由 `controlPanelController.clear()` 通知 Host 清空。
- `OpenJuggControlPanelAction` 位于 `ide_entry`，只调用 Host；JuggManager 自身不保存 Panel、事件枚举或 Sync taskId。
...
- Model 保留 Run Configuration、selected devices、package、changed files、baseline 与 deploy history 等 Context/Health 数据，Overview 不展示 context 摘要；Settings 使用原生分组、复选框和文字 action。Quick deploy、Embed APK、Project Kotlin 与按设备 compat 只在 Gradle 注入能力开启时展示，Backup classpath 只在当前环境可用时展示；SO hot update 始终展示且默认关闭，开关变化时提示下一次 Run 执行 `pm clear` 并强制重装，使旧 SO 补丁失效；Embed APK 与 Backup classpath 保留确认流程，后者切换成功后删除 deploy history。
