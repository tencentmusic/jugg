...
- More Options 下拉与工具入口：`ide/logic/MoreOptionsManager.kt`、`ide/ui/InstallJuggSkillsDialog.kt`。  
  ...
  - 安装弹窗默认开启 agent hooks 安装，按客户端注入 start/stop/edit/command 事件：Codex/Claude/CodeBuddy 使用 `UserPromptSubmit`、`Stop`、`PostToolUse`、`PreToolUse`；Gemini 使用 `BeforeAgent`、`AfterAgent`、`AfterTool`、`BeforeTool`；Cursor 使用 `beforeSubmitPrompt`、`stop`、`afterFileEdit`、`beforeShellExecution`。其中 Codex/Claude/CodeBuddy 的 tool hooks 使用精确 matcher（edit: `Edit|Write|MultiEdit|apply_patch`，command: `Bash`）；Gemini 使用精确 matcher（edit: `write_file|replace`，command: `run_shell_command`）；Cursor 维持 `*` matcher。hook 脚本与运行态状态均放在 `~/.jugg/skills/hooks`，会话状态文件位于 `~/.jugg/skills/hooks/.state`。start hook 只记录触发日志，不调用 Jugg CLI、不写状态；edit hook 记录本次 Agent 会话使用过写入工具及 `lastWriteTimeMs`，不解析修改文件；command hook 会把 shell 命令以单行形式写入 debug log，并仅对低误判 shell 源码写入模式补记本会话写入时间。raw Gradle guard 与 stop hook 均要求本会话存在未被 Jugg 覆盖的写入（`lastWriteTimeMs` 晚于 `status.lastCompileTime`；旧状态缺少写入时间时保守视为未验证）且 Jugg 仍存在 pending changes 才阻断，避免其他会话或构建产物 pending changes 误阻断；首次阻断会把当前 `Jugg status` 以 plain key-value 附在阻断提示中（包含 `enabledAndroidTest` 等字段），供 Agent 复用上下文；raw Gradle guard 仍按 pending 文件指纹隔离首次阻断，Codex/Claude 二次放行提示通过 `systemMessage` 输出；Codex/Claude 重复 stop 且 pending 仍存在时也通过 `systemMessage` 输出 warning。
