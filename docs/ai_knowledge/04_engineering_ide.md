...
Composite build 的自动生成身份沿用 project info 中的 `moduleStdPath + buildVariant`，Gradle task 和稳定 UUID 均由该身份生成；已有配置优先按解析后的 module/variant 复用，无法解析时按 `moduleName + variant` best-effort 匹配。Android model suggestion 中的 Gradle path 保留原始 segment，包含点号的模块名不会被拆成多级 path；included build 仍保留 build identity。
