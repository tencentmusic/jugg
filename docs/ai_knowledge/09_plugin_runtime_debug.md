...
| Kotlin `INTERNAL_ERROR` 且栈含 `Could not find installation home path` / shaded `PathManager.getHomePath` | 核对是否使用项目 Kotlin compiler、`idea.home.path` 是否指向含 `bin/idea.properties` 的真实 IDE 安装目录，以及 `Set idea.home.path from host IDE home` 或无法解析的 debug 日志。编译器可能在读取 plugin descriptor 的 build number 时触发该路径；兼容失败时应保留原异常与 Gradle 回退 | `KotlinCompilerHostCompat`、`KotlinCompilerInvoker`、`KotlinCompilerProcessRunner`；`02_compile_source.md` |
