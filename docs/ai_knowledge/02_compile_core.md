...
- 用户或基线强制：Force Gradle、BuildTarget 切换、compile command 变化、project info 不可用；外部源码变化遇到远程编译、无法安全派生 task 的非标准 Gradle command、任一 external 输入缺少 metadata/task/产物契约、metadata 标记不支持、Dart/C/C++ 源码或配置输入被删除，或上一轮已收集的 native 产物本轮不再产生。Flutter asset 删除及其产物集合缩小不触发回退。
