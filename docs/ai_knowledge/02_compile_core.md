...
| `ExternalBuildCompiler` / `ExternalBuildTaskRunner` | `main/.../compiler/external/` | 对 Dart/C/C++ 变化执行当前 variant 的 Flutter/native Gradle task，并把新 assets/`.so` 转为既有增量产物；命中工程声明的前置 codegen glob 时，同一轮先跑该 task，再把声明目录里的 Kotlin/Java 交给 `SourceCompiler` |
...
JuggCompiler.doCompile(task)
  ...
  -> ExternalBuildCompiler：Dart/C/C++ 变化执行 Flutter/native Gradle task
     -> 命中 `ExternalBuildInfo.prerequisites` 的 glob 时，同一轮 Gradle 先跑声明的 codegen task
     -> Flutter assets 与 Flutter/C++ .so 转为 Asset/NativeLib
     -> 声明目录内相对 Gradle 执行前 mtime/length 变化过的 Kotlin/Java 交给后续 SourceCompiler
...
- 外部源码根可位于 Android 模块目录之外，因此会额外加入文件扫描根；`.dart_tool`、`.cxx`、`.externalNativeBuild`、模块 build directory，以及 metadata 声明的 `excludedDirs`（Flutter SDK 根、pub cache 根等）始终排除，避免把生成文件或依赖缓存重新识别为源码变化。目录规则只处理当前存在的普通文件：已识别的 Dart/C/C++ 源码、Flutter asset 或配置输入被删除时都直接忽略，不根据历史状态恢复输入类型，也不做删除适配。例外是 `ExternalBuildInfo.prerequisites` 的 trigger glob：删除仍匹配该 glob 的路径会生成 `ExternalBuildSource`，由预检回退完整 Gradle。旧 Native 代码或 Flutter asset 需要一次完整 Run 才能从设备移除，移动进入监控范围的新路径按普通新增事件识别。
- 删除事件只按路径移除此前登记的待编译项；已不存在的文件默认不会转成 `ChangedFile`，也不会生成 class、resource、asset 或 Manifest 的移除数据。前置 codegen 的 trigger 文件删除除外，见上条。删除本身因此不会让增量编译失败或自动回退，设备继续保留已安装 APK 和既有 overlay 中的旧内容。重命名会被拆成旧路径删除和新路径新增/修改，只有新路径能够进入编译。需要让旧内容真正消失时，才通过完整 Gradle build 刷新 APK 基线。
