...
| 类/接口 | 文件 | 作用 |
|---|---|---|
| `JuggProjectInfo` / `ModuleInfo` | `main/src/main/java/com/sickworm/intellij/jugg/project/info/JuggProjectInfo.kt` | Gradle 项目/模块快照，记录 AGP R8 classpath、source/res/manifest/classpath/dependency/applicationId/androidTest 与外部构建信息 |
...
| `~/.jugg/locks/standalone.launch.lock` | Python CLI | standalone 自动拉起的进程级互斥锁；锁内二次发现 Runtime，避免不同项目并发创建多个 daemon |
...
| `kotlinJvmTarget` / `kotlinFreeCompilerArgs` / `kotlinPluginOptions` | Kotlin 编译任务的有效 JVM target、附加编译参数与 Gradle subplugin 参数；`kotlinFreeCompilerArgs` 已包含由 typed `compilerOptions.optIn` 转换出的 `-opt-in=<marker>` |
...
`kotlinJvmTarget` 与 `kotlinFreeCompilerArgs` 优先从当前变体 Kotlin 编译任务的 `compilerOptions` 读取，以兼容 Kotlin 2.x typed compiler options；typed `compilerOptions.optIn` 的 marker 会转换为 `-opt-in=<marker>` 后按原顺序合并，旧版 Kotlin Gradle Plugin 才回退到 `kotlinOptions`。`kotlinPluginOptions` 从 task 的 `KotlinCompilerPluginData.options.arguments` 读取；消费时优先使用当前 compilation，只有参数为空才回退最近父模块，禁止合并多个 compilation 的 option 集合。Kotlin task 发现兼容 AGP 9 Built-in Kotlin 与 KMP Android task `compileAndroidMain`。
...
启用 minify 的 buildType 会额外得到 `GradleApplicationInjector` 生成的 `build/jugg/proguard-rules.pro`（经 `proguardFile()` 附加）。该文件除 keep Application 子类外，还必须为注入 runtime 引用但未随包发布、宿主也无从提供的类型补 `-dontwarn`：私有 Kotlin runtime 的 `...runtime.kotlin.Metadata`、`...runtime.kotlin.jvm.JvmStatic`、`...runtime.kotlin.jvm.internal.markers.KMappedMarker`，以及未重定位的 `org.jetbrains.annotations.NotNull` / `Nullable`（Java-only 或没有 annotations 依赖的 App 上 R8 会以 missing class 失败）。更换预处理 Dragonfly JAR 后必须重新核对这批规则：Debug 不经过 R8，只跑 Debug 场景覆盖不到该路径。
...
外部构建信息以当前 variant 的真实 Gradle task 为准，统一表达为“产生最终 native 产物的 task + native 输出位置”，并用一个 `File` 记录输出位置、运行时按 file/directory 分派。Flutter 先定位 `compileFlutterBuild<Variant>`，再按 Flutter 版本选择 native task：旧版为唯一的、依赖该 compile task 的 `Jar` 类型 `packJniLibsflutterBuild<Variant>` 或 `packLibsflutterBuild<Variant>`，native 输出取 `archiveFile` / `archivePath`；Flutter 3.x 为依赖该 compile task 的 `copyJniLibsflutterBuild<Variant>`，native 输出取任务真实 `destinationDir`，不按 `generated/jniLibs` 等固定路径猜测。Flutter 另记录工程根和 assets 输出目录，因为 Dart 增量必须交付 `flutter_assets`；C++ 的 native 输出是 `merge<Variant>NativeLibs` 的真实输出目录，assets 输出为空。Gradle 快照负责提供这些配置事实，真正的 task 只在对应 Dart/C/C++ 文件进入增量编译后执行。字段通过 Gradle JSON 序列化并在合并时覆盖 IDE 空值。
...
```text
jugg CLI 发现目标项目没有 owner
  -> 优先复用任意已运行的 standalone Runtime
  -> 没有 standalone 时通过全局 launch lock 启动 jugg-standalone --project-dir <projectDir>
  -> JuggDaemon
     创建 StandaloneProjectRegistry
     为启动项目调用 StandaloneJuggRuntimeAssembler
  -> 首个合法项目级 MCP 请求
     校验工具、参数和 projectDir
     未注册时调用 StandaloneJuggRuntimeAssembler
  ...
  -> McpLocalServer
     ...
     compile / deploy / gradle-build / status / restart / devices 按 projectDir 路由到项目 Runtime
```

`version`、`list-projects` 等全局请求不会注册项目；非法工具、非法 schema 或不存在的项目目录也不会产生注册副作用。项目初始化按 canonical projectDir 独立协调：同项目并发请求共享一次初始化，不同项目并行初始化，慢项目不会阻塞已注册项目的请求。构造失败会对称关闭该项目已创建的 monitor、scope、task manager 和日志资源，再返回 `PROJECT_NOT_INITIALIZED`；已经注册的项目继续工作，失败项目也可安全重试。CLI 在目标项目尚未出现在 `list-projects` 时保留 pending projectDir，首个项目请求等待自动初始化并沿用 standalone 启动 heartbeat。

`jugg stop` 不进入上述 MCP 路由。CLI 直接同步调用稳定 standalone launcher 的 `--stop-all` 控制模式；bootstrap 在加载 active Runtime JAR 前，通过 `ProcessHandle` 按同一 Jugg 根目录匹配全部 `StandaloneBootstrap` 进程。平台支持正常终止时先请求正常退出，5 秒后仍存活则强制终止；不支持的平台直接强制终止。该路径可处理端口尚未 ready 的启动卡顿，同时不会停止 IDEA Runtime、其他 Jugg 根目录的进程，也不会删除项目持久化状态。

Step 11 已由 `StandaloneProjectServices` 组装 Gradle-only model、Compile Context、历史恢复、WatchService/Git reconcile、共享 `JuggCompilerHelper` 与 `JuggDeployerHelper`。WatchService 优先使用 JDK `FILE_TREE` 对工程根注册单个原生递归 WatchKey，不支持时回退逐目录注册；两条路径都忽略 `.git`、`.gradle`、`.idea` 和 `build` 子树事件。启动期若因文件句柄等原因失败，会关闭已创建的 watcher、注册 Git listener 并立即执行一次 Git refresh，后续 `status`、`compile`、`deploy` 也会继续通过 Git refresh 恢复变更。进程级 capability 为 `version`、`list-projects`、`compile`、`deploy`、`gradle-build`、`get-compile-status`、`status`、`restart`、`report-prepare`、`report-upload`、`devices`；首次 build 命令会根据 Gradle project info 按需创建当前 profile。`gradle-build` 建立 baseline 后继续安装或部署，`deploy` 负责普通增量编译与部署，`restart` 未传 serial 时重启全部在线目标设备，`devices` 通过项目级 `IDeployTargetManager` 返回在线设备。当前 profile 为 remote 时，standalone 在 Gradle full build/fallback 中复用 IDEA 的 `RemoteGradleCompileClient`、SSH/iFT 连接、文件同步和产物拉取链路；`rsync` 与 iFT 仍是 standalone 所在主机需要提供的外部工具。增量编译、设备操作与远程构建前的 project info Gradle dry-run 仍在该本机执行。standalone 不提供认证弹窗，必须在配置中预先提供 SSH 凭据或完成 iFT 认证；否则 compile job 返回 failed，清理终端输出与取消监听器，并给出非交互认证提示。Runtime 构造期的 config/history/context 恢复与每次完整 compile/deploy 链都持有同一项目写锁；standalone 项目写事务同时计入 daemon activity。空闲 `status` 仅在非阻塞取得项目锁后执行 owner 恢复、Git refresh 和一致性快照；同 Runtime 正在编译或项目锁正由其他写事务持有时，立即返回当前真实只读快照，不刷新或写入文件状态。运行期间检测到 IDEA/standalone owner 变化后，当前 Runtime 会在下一次成功取得项目写锁的业务链或 status snapshot 开始前重新加载 Compile Context、history、APK 与 Git 文件状态，并使 deployment memory cache 失效。
...
daemon 的 idle deadline 只由外部 MCP 请求刷新；WatchService、后台轮询和 update check 不刷新。达到 4 小时时，若存在 compile/deploy/Gradle job、项目写事务或更新下载，则每 1 分钟复查，条件解除后停止 MCP Server 并 dispose 全部项目 Runtime。用户也可通过 `jugg stop` 结束同一 Jugg 根目录下的全部 standalone daemon；支持正常终止的平台会先触发 JVM shutdown hook，超时后再强制终止，Windows 等不支持正常终止的平台直接强制终止。
...
- `ModuleBuildPathInfo.allClassPath` 只包含模块的普通 Gradle 输出，不包含任何 Gradle R.jar：R provider 由 `BaseCompileContext` 按模块身份选择并追加，避免同一 module 同时暴露 modern 与 legacy R 布局。synthetic androidTest 虽然建模为 `Library`，但必须使用自身 test variant 的 aggregate R，不能回落到 owner module 的主 variant R。`allBuildPaths` 同时包含 aggregate 目录和 `compile_r_class_jar`，保证远端 full build 后两种集合都能同步回本地。
