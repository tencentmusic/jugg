...
Jugg Wiki 以中文 `/zh/` 页面为唯一内容基准，英文根路径是中文页面的严格镜像：

- 去掉 `zh/` 前缀后，中英文 Markdown 路径集合必须完全一致，包括 dev-only 页面。
- 页面类型、章节顺序、表格行、提示块、代码块和相关页面结构保持一致。
...
- 修改事实、结构、边界或链接时，先更新中文，再在同一 diff 和 commit 中更新英文。
- 只有英文拼写、语法或自然度修正可以只修改英文；这类修改不得改变事实、结构、路由和链接。

中译英直接以中文页面为准，不重新核对实现。翻译可以按自然英文调整句式，但不得增加、删除、重排或弱化中文页面中的事实与边界。发现中文内容矛盾、链接失效或原意无法确定时，先报告问题，不在英文版自行补充解释。
