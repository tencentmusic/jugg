...
6. 你优先使用了 `layout_verify` 和 `eval_view` 吗？如果某些检查跳过了这两个工具，请逐条说明原因。
7. 如果你使用了 `screenshot` 作为判定依据（而非仅作为辅助截图），是因为 `layout_verify` / `eval_view` 无法覆盖该属性吗？请说明具体是哪个属性。
8. 你是否存在"截图看起来对了就 PASS"的情况？有没有每个判定都有 `layout_verify` / `eval_view` 的数据层数值支撑？
...
15. 对于 §1.4 列出的 `layout_verify` 不支持的属性（maxLines / ellipsize / cornerRadius 等），你用 `eval_view` 补充了吗？还是直接跳过了？
...
21. Report 的 "Actual" 列是来自 `layout_verify` / `eval_view` 的工具输出，还是你自己的目测估算？
