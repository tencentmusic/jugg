...
| 项目模型 | `JuggProjectInfo`, `ModuleInfo`, `ComposeResourceInfo`, `ComposeResourceSupportStatus`, `ComposeResourceDirectory`, `ModuleBuildPathInfo`, `JuggPathManager`, `JuggGlobalPathManager`, `ModuleApkBelongs`, `HotUpdateLoadManifest` | `project/data`, `project` | 根快照引用项目 AGP R8 分发包；模块快照保存路径、依赖、Kotlin common roots/fragment graph、Compose support 状态及资源根；项目信息读取/序列化；项目级路径与 `~/.jugg` 全局路径统一管理；模块到 APK 归属封装；IDEA/standalone 共享 hot update 完整加载快照 | 稳定 | 2026-08-05 |
| 任务与执行锁 | `TaskRunnerManager`, `ExecutionLockManager.kt` | `project` | TaskRunner 是唯一公开锁入口；`isProjectWrite` / `isGlobalWrite` 互斥选择项目锁或固定 `~/.jugg/locks/global.lock`，TaskRunner 创建前的基础设施使用静态全局写入口。Runtime identity、owner、锁接口和文件锁实现均为内部声明，底层支持进程内重入、NIO 跨进程锁、owner metadata、后台 Job 跟踪与 dispose 取消 | 开发中 | 2026-07-15 |
