...
| Gradle 信息读取 | `GradleProjectInfoReader`, `ProjectInfoSerializerInGradle`, `GradleDependencyDiffer` | `gradle/script` | 通过 Gradle 反射读取模块信息；从 Android Kotlin task 的 commonSourceSet 配置结构或 FileTree relative path 收集 authoritative common roots；严格读取 Compose resource task metadata 并同步到生成 init script | 稳定 | 2026-07-26 |
