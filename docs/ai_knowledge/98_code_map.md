...
| 项目运行基础设施 | `JuggPathManager`, `JuggGlobalPathManager`, `JuggSettings`, `JsonRuntimeSettingsRepository`, `ProjectCustomConfigManager`, `LastCompileProjectRegistry`, `HotUpdateLoadManifest` | `ide/bean`, `project/runtime` | 项目级与全局路径；`JuggSettings` 保留在原 `ide/bean` 包并自动加载 `~/.jugg/settings.json` effective settings，随可切换 global root 刷新缓存；`ProjectCustomConfigManager` 隐藏文件存储并统一应用 server/local custom config；跨项目运行状态和 IDEA/standalone 共享 hot update 完整加载快照。IDEA 旧设置转换位于 `idea/.../project/runtime` | 开发中 | 2026-07-20 |
| 编译上下文 | `BaseCompileContext`, `CompileContextManager`, `ICompileEnvironmentSource`, `IdeaProjectModelSource`, `IdeaCompileEnvironmentSource` | `compiler/context`, `idea/.../compiler/context` | Compile Context 核心位于 main；IDEA 侧保留 module/JDK/source root model 与按使用时读取的环境适配 | 开发中 | 2026-07-17 |
| 任务与执行锁 | `TaskRunnerManager`, `ExecutionLockManager.kt` | `project/runtime` | TaskRunner 是唯一公开锁入口；`isProjectWrite` / `isGlobalWrite` 互斥选择项目锁或固定 `~/.jugg/locks/global.lock`，TaskRunner 创建前的基础设施使用静态全局写入口。Runtime identity、owner、锁接口和文件锁实现均为内部声明，底层支持进程内重入、NIO 跨进程锁、owner metadata、后台 Job 跟踪与 dispose 取消 | 开发中 | 2026-07-16 |
| 项目级部署缓存 | `JuggDeploymentCacheStore`, `JuggDeploymentService` | `deploy/cache`, `idea/.../deploy/run` | `<projectDir>/build/jugg/deploy_cache` 下的磁盘 snapshot + 项目 Runtime 本地 memoryCache；项目锁内同步刷新、临时文件 flush + 原子替换 | 开发中 | 2026-07-14 |
...
| Gradle 编译客户端 | `LocalGradleCompileClient`, `RemoteGradleCompileClient`, `CopyGeneratedSourceHelper`, `GradleWrapperRepairer`, `ApkLookupPlanner`, `CmdExecutor`, `ProcessOutputReader` | `gradle/compile` | 本地/远端 Gradle 构建执行；远端产物中的 generated/custom sync 文件回写本地；Windows 命令输出按行严格校验 UTF-8，失败回退 GBK；`GradleWrapperRepairer` 在已有 wrapper properties 时补齐缺失 wrapper 启动文件，并在 Windows 远程编译同步前将 Unix `gradlew` 的 CRLF 转为 LF；AndroidTest 下区分 required app/app-test APK 与 optional history library Test APK 收集 | 稳定 | 2026-08-05 |
...
| Runtime / hot update | `RuntimeInfo`, `JuggHotUpdateManager`, `IdeaHotUpdateCoordinator`, `JuggHotUpdateBootstrap` | `project/runtime`, `server`, `idea/.../server`, `idea/.../loader` | Host-neutral runtime 信息；共享 hot update 下载校验与发布；IDEA 检查安装编排；Loader 启动前无锁只读 manifest | 开发中 | 2026-07-21 |
...
| 入口 | 文件路径 | 说明 |
|------|----------|------|
| IDEA 项目协调 | `idea/src/main/java/com/sickworm/intellij/jugg/JuggManager.kt` | 接收 Sync/Run/UI/MCP 事件，注入 IDEA runtime metadata，管理配置刷新、历史恢复、Compile Context 关联、IDEA monitor 接线、Control Panel 和 dispose；文件变化处理已委托共享 `FileChangeManager` |
