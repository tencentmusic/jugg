...
| 外部源码构建 | `ExternalBuildCompiler`, `ExternalBuildTaskRunner`, `resolveExternalBuild` | `compiler/external` | Dart/C/C++/Flutter asset/CMake 配置输入变化后执行当前 variant 的外部 Gradle task，刷新定向 project-info metadata，并把 native 与 Flutter assets 产物接入既有增量部署 | 稳定 | 2026-09-14 |
| 源码编译 | `SourceCompiler`, `JuggAptCompiler`, `IJuggAptProcessor`, `JavaCompiler`, `KotlinCompiler`, `KotlinCompilerInvoker`, `AndroidJarClasspathRetry`, `KotlinComplementaryFilesCache`, `K2JVMCompilerIsolate`, `TransformerCompiler`, `HiltAndroidEntryPointTransformer`, `DexCompiler`, `DexFileMaker` | `compiler/source`, `compiler/source/apt`, `compiler/source/kotlin` | Java/Kotlin 编译与 DEX 生成；D8 前通过 `ClassPreparation` 共享 class analysis，Hilt transformer 恢复 Android 入口父类、super 调用与 Receiver 注入；ROM hidden API 被 SDK `android.jar` 遮蔽时按源文件后置 classpath 重试一次；Kotlin 编译保留 Gradle subplugin 与 KMP 语义 | 稳定 | 2026-09-10 |
...
| 部署编排与数据模型 | `IDeployTargetManager`, `JuggDeployOrchestrator`, `JuggDeployerHelper`, `JuggDeployTask`, `JuggDeployer`, `DeployStateRecover`, `DeployRetryHandler`, `JuggDeployData`, `LaunchResult` | `deploy`, `deploy/run`, `deploy/direct`, `deploy/hotreload`, `deploy/flutter` | 单设备 lifecycle、recover、retry、Apply Changes 与 Direct Overlay 已共享；`AppSandboxExecutor` 和 `DirectAppSandboxDeployTransport` 为 `run-as` 不兼容应用提供直接部署，sandbox 完全不可用时由 `RootlessCompatDeployStaging` 暂存、App 侧 `RootlessCompatDeployImporter` 导入；`FlutterJitCacheInvalidator` 清理 JIT runtime timestamp；IDEA Host 还可为普通 App install/reinstall 执行自定义 APK 安装脚本 | 开发中 | 2026-09-23 |
...
| 项目信息 | `JuggProjectInfo`, `ModuleInfo`, `ExternalBuildInfo`, `ExternalBuildInputDir`, `ExternalBuildInputFilterRule`, `ExternalBuildInfoRequest`, `ExternalBuildInfoUpdate`, `ComposeResourceInfo`, `ComposeResourceSupportStatus`, `ComposeResourceDirectory`, `ModuleBuildPathInfo`, `IProjectModelSource`, `GradleProjectModelSource`, `ProjectInfoSerializer`, `JuggProjectInfoMerger`, `ModulePathMergePolicy` | `project/info` | 模块快照保存路径、依赖、Kotlin/Compose metadata，以及 Flutter/C++ task、带文件类型规则的递归输入根、配置输入、排除目录、native 输出与 Flutter assets；request/update 模型承载单次 Gradle invocation 的定向 metadata 刷新 | 开发中 | 2026-09-23 |
...
| 项目运行基础设施 | `JuggPathManager`, `JuggGlobalPathManager`, `JuggResourceManager`, `JuggSettings`, `JsonRuntimeSettingsRepository`, `ProjectCustomConfigManager`, `CliRunConfiguration`, `CliRunConfigurationStore`, `LastCompileProjectRegistry`, `HotUpdateLoadManifest` | `ide/bean`, `project/runtime` | 项目级与全局路径；`JuggResourceManager` 将 classpath resource root 固定映射到 `~/.jugg/resources`，并在固定全局写锁内通过临时文件原子覆盖；`JuggSettings` 保留在原 `ide/bean` 包并自动加载 `~/.jugg/settings.json` effective settings；`ProjectCustomConfigManager` 统一应用 server/local custom config；`CliRunConfiguration` 以独立 UUID JSON + 当前指针共享 IDEA/standalone build profile，并由 Gradle project info 确定性推断默认配置；跨项目运行状态和 hot update 完整加载快照共享。IDEA 旧设置转换与 run configuration 同步位于 `idea/.../project/runtime` | 开发中 | 2026-09-05 |
...
| 项目级部署缓存 | `JuggDeploymentCacheStore`, `JuggDeploymentService` | `deploy/cache`, `deploy/run` | `<projectDir>/build/jugg/database/deploy_cache.db` 磁盘 snapshot + Runtime 本地 memoryCache；项目锁内刷新并原子替换，工程目录变更时随 `database/` 清除；Service 按 owner 或内容 generation 变化失效并重载 | 开发中 | 2026-09-16 |
...
| Gradle 信息读取 | `GradleProjectInfoReaderManager`, `GradleProjectInfoReader`, `NativeBuildMetadataReader`, `NativeStripConfigCache`, `GradleVariantCollector`, `ProjectInfoSerializerInGradle`, `GradleDependencyDiffer` | `gradle/script` | 通过 Gradle 反射读取模块、Kotlin/Compose metadata 与 Android plugin classloader 中 D8；收集当前 variant 的 Flutter/C++ 外部构建 task、带规则的递归输入根、配置输入、排除目录、native 输出和 Flutter assets，并支持 external task 后的定向 metadata 输出；`NativeStripConfigCache` 为 Configuration on Demand 保留 APK owner 的 strip 配置 | 稳定 | 2026-09-23 |
...
| MCP 工具 | `McpToolActionRegistry`, `CompileJobManager`, `DeviceSelectionResolver`, `GetCompileStatusMcpToolAction`, `PrepareIssueReportMcpToolAction`, `UploadIssueReportMcpToolAction`, `LayoutDumpHelper`, `LayoutHtmlConverter`, `WaitLogsMcpToolAction`, `CrashDetector`, `LastDeployTimestampRegistry` | `ai/mcp/actions`, `ai/mcp/util` | 工具注册、异步编译状态管理、两阶段诊断包确认上传与请求级 serial 精确设备选择；设备 action 从当前 Runtime 的 `IDeployTargetManager` 创建 ADB 适配器，不依赖进程级 `PlatformApi`；`LayoutDumpHelper` 封装 layout_dump 核心逻辑（设备解析、px→dp、公开 HTML 输出、内部 JSON 文件），`LayoutHtmlConverter` 将 JSON 视图树转为精简 HTML（含虚拟节点裁剪）；`WaitLogsMcpToolAction` 阻塞式等待 App 日志（marker/crash/timeout 判停）；`LastDeployTimestampRegistry` 按项目与设备记录 deploy/restart 日志起点 | 稳定 | 2026-09-08 |
| Standalone 项目运行域 | `StandaloneProjectServices`, `StandaloneConfigurationRunner`, `StandaloneProjectInitializer`, `StandaloneDeployTargetManager` | `cmd_line/.../standalone` | 恢复项目上下文，串联 WatchService/Git、共享增量/Gradle 编译、共享部署、MCP job 取消/轮询和真实 adb 设备边界；deploy/status/restart/devices 支持请求级 serial 覆盖 daemon 启动时的 `ANDROID_SERIAL`，UI/日志等未注册能力仍不扩展；`cmdline-distribution.gradle` 从实际 runtimeClasspath 生成 SHA-256 内容寻址的跨平台 Bundle，并执行 Java 11 发行门禁 | 开发中 | 2026-09-08 |
| Standalone 发行与启动 | `StandaloneRuntimeInstaller`, `StandaloneBootstrap` | `cmd_line/.../standalone`, `cmd_line/standalone_bootstrap/` | 校验 JDK/Python/Bundle，原子安装共享 JAR、版本化 tooling/CLI 并最后提交 active manifest；外部 Bundle 始终完整自包含，IDEA 内嵌副本可按插件 `jugg/lib` 的 SHA-256 精确匹配裁剪；Bundle 安装入口和稳定 launcher 每次按有效 `JAVA_HOME`、PATH 动态选择 Java；bootstrap 在加载 Runtime JAR 前支持停止同一 Jugg root 下的全部 standalone daemon，安装提交后也强制停止同根目录全部旧 daemon，并在停稳后删除旧版 `~/.jugg/runtime` deployer 资源；启动失败保留 active manifest 并直接返回异常 | 稳定 | 2026-09-05 |
| AI 技能安装 | `JuggSkillInstaller`, `JuggHookInstaller`, `CcSwitchCommonConfigGuideExporter`, `PythonRuntimeResolver`, `CodexPermissionRuleInstaller`, `JuggCliAutoUpdater`, `ClientSetupDocExporter`, `IAgentInstaller`, `agents/*` | `ai/skills` | 安装/更新 `jugg-android-dev-loop` skill、CLI 与 hooks（资源来源 `docs/skills/*.zip`）；CLI/hooks 安装先校验 Python 3.7+，CLI wrapper 运行时按 `python3`、`python`、`py -3` 顺序校验并回退，避免前一命令存在但版本过低导致启动失败；成功安装 Claude hooks 后，安装结果关闭再异步检查桌面版 / `cc-switch-cli` 共用的配置目录，用户确认后仅导出 Jugg Claude hooks 至 `~/.jugg/cc-switch` 并打开文件，不读写 CC Switch provider 或数据库；Windows CLI 安装由 `JuggSkillInstaller` 写入用户级 PATH，macOS/Linux 创建 `~/.local/bin/jugg` symlink；Codex skill 安装时同步写入 `rules/default.rules` 的 Jugg CLI `prefix_rule`，并导出 `agent_setup.md`；`IAgentInstaller` 统一描述各 agent 的 skill/hook/rules 安装目标，Installer 仅保留调度 | 稳定 | 2026-09-05 |
...
| Runtime / hot update | `RuntimeInfo`, `RuntimeOwnerStore`, `JuggHotUpdateManager`, `IdeaHotUpdateCoordinator`, `JuggHotUpdateBootstrap` | `project/runtime`, `server`, `idea/.../server`, `idea/.../loader` | Host-neutral runtime type/version/host/build，由各 Host 的 `IPlatformApi` 单点提供；持久化 last owner；共享 hot update 下载校验与发布；IDEA 检查安装编排，只有真实 IDEA 热更新会发布 `load_manifest.json`，standalone 使用独立 manifest；Loader 启动前无锁只读 manifest，`PlatformApi`、`IPlatformApi`、`IdeaPlatformApi` 和接口 JVM 描述符中的直接 Jugg 类型固定由宿主 ClassLoader 提供，使 MCP HTTP 与热更新运行域共享同一 Host bridge；Bootstrap 跨 classloader API 仅暴露 JDK 平台类型 | 开发中 | 2026-08-15 |
| 工具模块 | `Aapt2DaemonInvoker`, `ApkFileModifier`, `CustomApkSignScriptRunner`, `GitManager`, `JuggLogger`, `JuggServer`, `JuggEventLocalStore`, `IssueReportBundleBuilder`, `IssueReportBundleReader`, `IssueReportUploader`, `ExpiredArtifactCleaner`, `PlatformApi` | `aapt2/`, `apk/`, `git/`, `logger/`, `server/`, `diagnostics/`, `project/`, `platform/` | 通用基础能力；`CustomApkSignScriptRunner` 可按 Run Configuration 替换增量 APK 的默认 keystore 签名；report 事件写入 `~/.jugg/action.db`，问题诊断使用白名单包、manifest/SHA-256 上传前复核和独立单目标上传；MCP 与问题诊断临时产物分别保留 30 天和 7 天 | 稳定 | 2026-09-23 |
...
| Standalone Bundle 接入 | `idea/src/main/java/com/sickworm/intellij/jugg/ide/logic/StandaloneEmbeddedBundle.kt`, `StandaloneBundleInstallService.kt`, `idea/src/main/java/com/sickworm/intellij/jugg/server/IdeaHotUpdateCoordinator.kt` | 构建和热更新重装时生成差量 Bundle；安装前从插件 `jugg/lib` 按 SHA-256 恢复省略的 runtime/bootstrap JAR，再复用 standalone 安装事务 |
...
| 插件加载 | `idea/src/ide_entry/java/com/sickworm/intellij/jugg/loader/JuggLoader.kt` | 类加载隔离与桥接；`com.sickworm.intellij.jugg.ide` 保持稳定；Platform bridge 通过反射收集 `IPlatformApi` 直接 Jugg 签名类型并与实现一起固定在宿主，热更新业务仅通过该闭包跨边界 |
...
| 模块 | 目录 | 关键点 |
|------|------|--------|
| deploy_compat | `deploy_compat/interface/src/main/java/com/sickworm/intellij/jugg/deploy/api/`, `deploy_compat/*/src/main/java/com/sickworm/intellij/jugg/deploy/run/` | `deploy.api` 仅保存同名自有设备/APK/protobuf/logger 契约并保留 D8 字段重初始化状态；legacy、Quail 与 Rabbit 版本模块转换真实 Android 类型并持有实例级弱缓存，interface JAR 不提供 raw converter；owner-bound deploy 调用固定使用 priority compat，wrapper 隔离 deployer runtime 类型包迁移，`attachJavaDebugger()` 隔离 Java debugger API 迁移 |
| standalone_deployer | `deploy_compat/standalone_deployer/src/main/java/`, `deploy_compat/standalone_deployer/src/main/resources/deployer/quail/` | 固定 Quail 1 的 Java 11 deployer 最小闭包、`StandaloneApplyChangesExecutor`、真实 ddmlib `StandaloneDeviceManager`、严格向后兼容的 installer/protocol 资源；四 ABI 文件固定释放到 `~/.jugg/resources/deployer/quail` 单份目录，不打包或加载完整 `sdk-tools.jar` |
...
| cmd_line | `cmd_line/src/main/java/com/sickworm/intellij/jugg/cmdline/` | 现有 `CmdLine`/CI 命令保持不变；`standalone/` 由进程级 registry 承载多个 project runtime，合法项目请求按需注册，首次构建按需创建 profile，并提供 Gradle/增量 compile、deploy、status、两阶段 report 与懒加载 ddmlib 生命周期；`standalone_bootstrap/` 提供同 Jugg 根目录 stop-all |
...
- 查“APK 改写后签名失败”：从 `IncrementalDeployHelper.updateApk` -> `ApkFileModifier.insertAndResign` -> `CustomApkSignScriptRunner`。
