...
| 项目模型 | `JuggProjectInfo`, `ModuleInfo`, `ComposeResourceInfo`, `ComposeResourceSupportStatus`, `ComposeResourceDirectory`, `ModuleBuildPathInfo`, `JuggPathManager`, `JuggGlobalPathManager`, `ModuleApkBelongs` | `project/data`, `project` | 根快照引用项目 AGP R8 分发包，Gradle instrumentation code source 恢复为原始 buildscript artifact；模块快照保存路径、依赖、Compose support 状态/原因及尚不存在的默认/自定义配置根；项目信息读取/序列化；项目级路径与 `~/.jugg` 全局路径统一管理；模块到 APK 归属封装 | 稳定 | 2026-08-03 |
| 依赖变更 | `DependencyChangeManagerByGradle`, `DependencyChangeManagerBySync` | `project/dependency` | 依赖变更检测策略 | 稳定 | 2025-01-20 |
| Gradle 信息读取 | `GradleProjectInfoReaderManager`, `GradleProjectInfoReader`, `GradleVariantCollector`, `ProjectInfoSerializerInGradle`, `GradleDependencyDiffer` | `gradle/script` | 通过 Gradle 反射读取模块信息与 Android plugin classloader 中 D8 的 code source；instrumentation cache 路径从 buildscript classpath 恢复原始 artifact；legacy variant API 无结果时使用配置阶段 Android Components variant 名称回退，并供 AndroidTest task 选择复用；从 Android Kotlin task 的 commonSourceSet 结构或 FileTree relative path 收集 authoritative common roots；严格读取 Compose resource task metadata 并同步到生成 init script | 稳定 | 2026-08-03 |
