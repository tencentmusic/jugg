...
| 项目运行基础设施 | `JuggPathManager`, `JuggGlobalPathManager`, `JuggSettings`, `JsonRuntimeSettingsRepository`, `ProjectCustomConfigManager`, `CliRunConfiguration`, `CliRunConfigurationStore`, `LastCompileProjectRegistry`, `HotUpdateLoadManifest` | `ide/bean`, `project/runtime` | 项目级与全局路径；`JuggSettings` 保留在原 `ide/bean` 包并自动加载 `~/.jugg/settings.json` effective settings；`ProjectCustomConfigManager` 统一应用 server/local custom config；`CliRunConfiguration` 以独立 UUID JSON + 当前指针共享 IDEA/standalone build profile，并由 Gradle project info 确定性推断默认配置；跨项目运行状态和 hot update 完整加载快照共享。IDEA 旧设置转换与 run configuration 同步位于 `idea/.../project/runtime` | 开发中 | 2026-07-21 |
...
| 任务与执行锁 | `TaskRunnerManager`, `ExecutionLockManager.kt`, `RuntimeOwnerStore` | `project/runtime` | TaskRunner 是唯一公开锁入口；`isProjectWrite` / `isGlobalWrite` 互斥选择项目锁或固定 `~/.jugg/locks/global.lock`。当前持锁 metadata 保存于 `runtime.lock.owner.json`，上次 IDEA/standalone owner 独立原子保存于 `runtime.owner.json` 并生成 owner-change event；损坏的 last owner 按无历史 owner 自愈；CI 不参与 last owner 归属 | 开发中 | 2026-08-05 |
...
| cmd_line | `cmd_line/src/main/java/com/sickworm/intellij/jugg/cmdline/` | 现有 `CmdLine`/CI 命令保持不变；`standalone/` 新增 `JuggDaemon`、`StandaloneProjectRegistry`、`StandaloneJuggRuntimeAssembler`、MCP runtime 与 idle lifecycle；同一 distribution 额外生成 `jugg-standalone` launcher，均使用 Java 11；Python CLI 通过项目级 launch lock 去重并发自动拉起 |
