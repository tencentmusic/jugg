...
| 项目文件变化 | `ChangedFile`, `FileChangesHandler`, `IFileChangesHandler`, `IFileChangesDetector` | `project/change` | 项目文件变化的检测契约、过滤与可编译文件建模；IDE detector 实现在 `idea` 同名包 | 稳定 | 2026-07-16 |
| 项目运行基础设施 | `JuggPathManager`, `JuggGlobalPathManager`, `CustomConfigManager`, `LastCompileProjectRegistry`, `HotUpdateLoadManifest` | `project/runtime` | 项目级与全局路径、项目配置、跨项目运行状态，以及 IDEA/standalone 共享的 hot update 完整加载快照 | 稳定 | 2026-07-16 |
...
| 依赖变更 | `DependencyChangeManagerByGradle`, `DependencyChangeManagerBySync`, `GradleProjectInfoLocalFetchManager` | `project/dependency` | 依赖变更检测，以及不依赖 IDEA Project、通过共享 TaskRunner 保留项目锁的本地 Gradle project info 调度主体 | 开发中 | 2026-07-17 |
...
| 入口 | 文件路径 | 说明 |
|------|----------|------|
| IDEA 项目协调 | `idea/src/main/java/com/sickworm/intellij/jugg/JuggManager.kt` | 接收 Sync/Run/UI/MCP 事件，管理配置刷新、历史恢复、Compile Context 关联、文件变化、Control Panel 和 dispose |
