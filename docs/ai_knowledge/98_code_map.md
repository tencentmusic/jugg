...
| Standalone 发行与启动 | `StandaloneRuntimeInstaller`, `StandaloneBootstrap` | `cmd_line/.../standalone`, `standalone_bootstrap/` | 校验 JDK/Python/Bundle，原子安装共享 JAR、版本化 tooling/CLI 并最后提交 active manifest；固定 Java 11 bootstrap 按 active manifest 有序加载 Runtime，启动失败时保留 active manifest 并直接返回异常 | 稳定 | 2026-08-10 |
