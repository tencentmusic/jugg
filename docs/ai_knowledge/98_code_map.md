...
| 部署编排与数据模型 | `IDeployTargetManager`, `JuggDeployOrchestrator`, `JuggDeployerHelper`, `JuggDeployTask`, `JuggDeployer`, `DeployStateRecover`, `DeployRetryHandler`, `JuggDeployData`, `LaunchResult` | `deploy`, `deploy/run`, `deploy/direct`, `deploy/hotreload`, `deploy/flutter` | 单设备 lifecycle、recover、retry、Apply Changes 与 Direct Overlay 已共享；`AppSandboxExecutor` 和 `DirectAppSandboxDeployTransport` 为 `run-as` 不兼容应用提供直接部署，`FlutterJitCacheInvalidator` 清理 JIT runtime timestamp；IDEA Host 还可为普通 App install/reinstall 执行自定义 APK 安装脚本 | 开发中 | 2026-09-14 |
...
| 项目信息 | `JuggProjectInfo`, `ModuleInfo`, `ExternalBuildInfo`, `ExternalBuildInfoRequest`, `ExternalBuildInfoUpdate`, `ComposeResourceInfo`, `ComposeResourceSupportStatus`, `ComposeResourceDirectory`, `ModuleBuildPathInfo`, `IProjectModelSource`, `GradleProjectModelSource`, `ProjectInfoSerializer`, `JuggProjectInfoMerger`, `ModulePathMergePolicy` | `project/info` | 模块快照保存路径、依赖、Kotlin/Compose metadata，以及 Flutter/C++ task、输入根、配置输入、排除目录、native 输出与 Flutter assets；request/update 模型承载单次 Gradle invocation 的定向 metadata 刷新 | 开发中 | 2026-09-14 |
...
| Gradle 信息读取 | `GradleProjectInfoReaderManager`, `GradleProjectInfoReader`, `NativeBuildMetadataReader`, `GradleVariantCollector`, `ProjectInfoSerializerInGradle`, `GradleDependencyDiffer` | `gradle/script` | 通过 Gradle 反射读取模块、Kotlin/Compose metadata 与 Android plugin classloader 中 D8；收集当前 variant 的 Flutter/C++ 外部构建 task、递归输入根、配置输入、排除目录、native 输出和 Flutter assets，并支持 external task 后的定向 metadata 输出 | 稳定 | 2026-09-14 |
...
| 工具模块 | `Aapt2DaemonInvoker`, `ApkFileModifier`, `GitManager`, `JuggLogger`, `JuggServer`, `JuggEventLocalStore`, `IssueReportBundleBuilder`, `IssueReportBundleReader`, `IssueReportUploader`, `ExpiredArtifactCleaner`, `PlatformApi` | `aapt2/`, `apk/`, `git/`, `logger/`, `server/`, `diagnostics/`, `project/`, `platform/` | 通用基础能力；report 事件写入 `~/.jugg/action.db`，问题诊断使用白名单包、manifest/SHA-256 上传前复核和独立单目标上传；MCP 与问题诊断临时产物分别保留 30 天和 7 天 | 稳定 | 2026-09-01 |
