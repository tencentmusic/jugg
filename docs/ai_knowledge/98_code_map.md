...
| 部署编排与数据模型 | `JuggDeployOrchestrator`, `JuggDeployTask`, `JuggDeployer`, `DeployStateRecover`, `DeployRetryHandler`, `JuggDeployData`, `LaunchResult` | `deploy/run`, `deploy/direct` | 单设备 lifecycle、recover、retry、Apply Changes 与 Direct Overlay 已共享；IDEA 已接线，standalone Host 边界已具备，完整组装留待 Step 11 | 开发中 | 2026-08-06 |
...
| IDEA 部署 Host | `idea/src/main/java/com/sickworm/intellij/jugg/deploy/run/JuggDeployerHelper.kt`, `IdeaDeployEnvironment.kt`, `IdeaDeployDebugger.kt`, `run/instrument/LibraryTestApkBackfillHelper.kt` | 选择 install/embedded/incremental，提供 IDEA 设备、ADB、prompt、debugger、AndroidTest UI，并将单设备 lifecycle 委托给 `main` 的 `JuggDeployOrchestrator` |
...
| cmd_line | `cmd_line/src/main/java/com/sickworm/intellij/jugg/cmdline/` | 现有 `CmdLine`/CI 命令保持不变；`standalone/` 提供 daemon/project runtime、`StandaloneDeployEnvironment` 与懒加载 ddmlib 生命周期。本 Step 尚未开放 compile/deploy MCP 用户命令 |
