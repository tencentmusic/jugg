...
| 项目信息 | `JuggProjectInfo`, `ModuleInfo`, `ComposeResourceInfo`, `ComposeResourceSupportStatus`, `ComposeResourceDirectory`, `ModuleBuildPathInfo`, `ProjectInfoSerializer`, `JuggProjectInfoMerger`, `ModulePathMergePolicy` | `project/info` | 根快照引用项目 AGP R8 分发包；模块快照保存路径、依赖、Kotlin common roots/fragment graph、Compose support 状态及资源根；提供序列化、合并和模块身份策略 | 稳定 | 2026-08-05 |
...
| 编译上下文 | `BaseCompileContext`, `CompileContextManager` | `compiler/context`, `idea/.../compiler/context` | 编译期模块/APK/classpath 上下文，以及 IDE 侧 ProjectInfo 更新和 Compile Context 生命周期 | 稳定 | 2026-07-16 |
