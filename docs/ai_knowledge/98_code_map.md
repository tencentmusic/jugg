...
| Runtime / hot update | `RuntimeInfo`, `RuntimeOwnerStore`, `JuggHotUpdateManager`, `IdeaHotUpdateCoordinator`, `JuggHotUpdateBootstrap` | `project/runtime`, `server`, `idea/.../server`, `idea/.../loader` | Host-neutral runtime type/version/host/build，由各 Host 的 `IPlatformApi` 单点提供；持久化 last owner；共享 hot update 下载校验与发布；IDEA 检查安装编排；Loader 启动前无锁只读 manifest，`PlatformApi`、`IPlatformApi`、`IdeaPlatformApi` 和接口 JVM 描述符中的直接 Jugg 类型固定由宿主 ClassLoader 提供，使 MCP HTTP 与热更新运行域共享同一 Host bridge；Bootstrap 跨 classloader API 仅暴露 JDK 平台类型 | 开发中 | 2026-08-15 |
