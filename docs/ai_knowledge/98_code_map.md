...
| 外部源码构建 | `ExternalBuildCompiler`, `ExternalBuildTaskRunner` | `compiler/external` | Dart/C/C++ 变化执行当前 variant 的 Flutter/native Gradle task，并把 assets/`.so` 接入既有增量部署 | 稳定 | 2026-09-05 |
| 源码编译 | `SourceCompiler`, `JuggAptCompiler`, `IJuggAptProcessor`, `JavaCompiler`, `KotlinCompiler`, `KotlinCompilerInvoker`, `KotlinComplementaryFilesCache`, `K2JVMCompilerIsolate`, `DexCompiler`, `DexFileMaker` | `compiler/source`, `compiler/source/apt`, `compiler/source/kotlin` | Java/Kotlin 编译与 DEX 生成；Kotlin compiler plugin 参数与 plugin JAR 按同一 current-to-parent module 范围聚合，并按 plugin id 局部处理缺参或不支持参数的单次降级；D8 优先隔离加载项目 AGP 的 R8 code source；typed common sources 与普通 KMP complementary/fragment/baseline 隔离保持 Gradle 语义 | 稳定 | 2026-09-05 |
...
| 编译入口 | `idea/src/main/java/com/sickworm/intellij/jugg/compiler/JuggCompileHelper.kt` | 增量/Gradle 回退判定；把当前 Gradle command 传给外部源码构建；AndroidTest Gradle build 读取 library Test APK build history 并注入回放任务 |
