...
| 外部源码构建 | `ExternalBuildCompiler`, `ExternalBuildTaskRunner` | `compiler/external` | Dart/C/C++ 变化从全量基线派生当前 variant 的 Flutter native 输出/native merge task，并按 native 输出是归档还是目录分派收集 `.so`，与 Flutter assets 一起接入既有增量部署 | 稳定 | 2026-09-12 |
