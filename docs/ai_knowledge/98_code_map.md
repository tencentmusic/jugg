...
| Standalone 发行与启动 | `StandaloneRuntimeInstaller`, `StandaloneBootstrap` | `cmd_line/.../standalone`, `standalone_bootstrap/` | 校验 JDK/Python/Bundle，原子安装共享 JAR、版本化 tooling/CLI 并最后提交 active manifest；Bundle 安装入口和稳定 launcher 每次按有效 `JAVA_HOME`、PATH 动态选择 Java；提交成功后强制停止同一 Jugg 根目录的 standalone daemon，使下次 CLI 调用加载新 Runtime；启动失败保留 active manifest 并直接返回异常 | 稳定 | 2026-08-10 |
