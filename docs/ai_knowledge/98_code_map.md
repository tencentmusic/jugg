...
| rpc | RpcLocalServer, RpcCommand | rpc | 本地 RPC/命令注册 | 稳定 | 2025-01-20 |
...
- 新增关键类/入口时，同步更新本表。
- 如目录变更，请注明旧路径/新路径，避免索引漂移。
> 建议在 PR 检查项中加入“已同步 Code Map 状态/日期”。
>
> ---
>
> ## 六、未覆盖/待补清单（占位）
>
> > 如统计发现有代码未覆盖到文档/索引，请在此列出：模块/路径、缺失项、计划补齐日期与责任人。
>
> - TODO: （示例）`main/.../XxxNewFeature` —— 缺少专题文档和 Code Map 条目。
>
> > 更新节奏：每个小版本前核对一次；补齐后移除该占位或填入最新状态/日期。
