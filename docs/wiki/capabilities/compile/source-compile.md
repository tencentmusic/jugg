...
| Change ordinary logic in an existing Hilt Android entry point with Hilt `2.41` through `2.60.1` | Supported | When existing Hilt generated artifacts are available, restores the entry-point transform before DEX generation and preserves injection; see [Annotation processors](./annotation-processors.md) |
...
- Hilt entry-point transformation depends on entry-point parents and injection code generated by the latest complete Gradle/Hilt build. Changes to injection declarations or missing generated artifacts require a complete Gradle build; the current compilation fails explicitly when the required generated parent is missing.
