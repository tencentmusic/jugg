...
- When a resource patch replaces the AssetManager, the in-app runtime preserves shared resource paths already declared by the system. If some vendor WebView implementations still lack the corresponding package ID after initialization, the Agent remounts the path only after confirming that the system supplied the WebView APK and the ID is missing, and records the resource state before and after the correction.
