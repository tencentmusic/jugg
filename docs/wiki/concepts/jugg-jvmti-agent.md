...
- When a resource patch replaces the AssetManager, the in-app runtime preserves shared resource paths already declared by the system. If a vendor WebView still lacks its package ID in the Resources used by the actual lookup, the Agent records that lookup boundary and adds only WebView APK paths already loaded by the system to that AssetManager.
