...
## APK update requires re-signing
...
The signing identity must match the package already installed on the device, otherwise the update is rejected. When a Gradle `SigningConfig` cannot express a platform certificate, vendor key, or server-side signing flow, the Run Configuration can enable a custom APK sign script: Jugg passes the absolute path of the aligned temporary APK as the last argument, the project script signs it in place, and Jugg still runs `apksigner verify` before replacing the original APK. With a script enabled, the local signing configuration is no longer a precondition for APK update, and a failed script never falls back to local keystore signing. The full contract is in [Custom APK sign script](../capabilities/deploy/custom-apk-sign-script.md).
...
generate incremental Manifest or native library artifacts
  ...
  -> re-sign with the project signing configuration, or with the project script when a custom sign script is enabled
...
- [Custom APK sign script](../capabilities/deploy/custom-apk-sign-script.md)
