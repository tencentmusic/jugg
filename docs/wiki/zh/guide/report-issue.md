...
报告问题功能把本轮 Jugg 日志和设备错误日志打包，上传到固定的问题报告服务。它适合在增量编译失败、部署失败、运行结果不符合预期时使用。无论是否配置了 Jugg 后台或 Custom Server，上传目标都不会改变。

团队后台还可以开启失败日志自动上传。自动上传与本页的手工流程使用同一个固定服务，但内容更少，也不会弹出确认窗口。
...
确认窗口会展示固定上传地址 `https://jugg.sickworm.com/report_issue`。插件只向该 HTTPS 地址提交诊断包，不读取 Custom Server，失败后也不会切换服务器。选择本地保存后，系统文件管理器会选中新生成的 zip。

上传成功后，结果窗口会显示 8 位小写十六进制 Report ID。把它和复现步骤一起发给维护者。
...
自动上传包只包含最近两份经过脱敏的真实 Jugg 日志和 manifest，不包含工程快照、环境摘要、logcat 或 hook 日志。后台还可以通过 `autoUploadFailureLogsExcludeRegex` 排除已知错误；正则命中当前 Run 的最终错误摘要时不上传。空正则表示不过滤，非法正则会跳过本次上传。

自动上传异步执行，不弹窗、不重试；上传失败不会改变原本的编译部署结果。上传目标仍是 `https://jugg.sickworm.com/report_issue`，不读取 Custom Server。
