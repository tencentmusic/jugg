...
额外排除规则使用 rsync pattern，不是 gitignore：

- 推荐用分号分隔，旧的逗号和换行仍兼容。
- 规则相对于实际传输根目录。
- 以 \`/\` 开头表示锚定传输根。
- 不支持通过父目录路径越出同步根。
- 默认已排除常见 build、IDE 和 Git 对象目录，不必重复添加 \`*.class\`。

示例：

\`\`\`text
.git; /.git/; local-temp/**; **/*.dat
\`\`\`

多工程模式下规则仍按实际传输根解释，不会自动加当前项目路径前缀。
...
| 修改未上传 | 检查传输根和 Additional exclude patterns |
