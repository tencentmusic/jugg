---
title: Jugg 工作原理
description: 从常规 Run 的固定耗时出发，说明 Jugg 如何依托 Gradle 基线，用定制编译器、扩散编译和混合部署绕过资源链接、编译器加载和整包刷新的固定开销，以及这些机制的边界与代价。
status: active
tags:
  - concept
  - run
...
它解决的是日常 Run 中与本轮小改动不成比例的固定开销：资源 link、编译器加载、APK 打包安装和运行状态重建。Jugg 通过可信 Gradle 基线、局部编译、影响分析和混合部署绕过这些固定阶段，同时在状态不可信时回到 Gradle。

## 常规 Run 的固定耗时

Gradle 一次完整 Run 要处理整个工程语义：配置阶段、任务图、插件逻辑、依赖解析、源码编译、资源 link、打包、签名和安装。

问题在于其中相当一部分耗时与本轮改动大小无关。资源 link 通常要读取完整资源输入，APK 打包和安装偏向“重做整包”，编译器进程的启动与 classpath 加载也按工程规模收费。在百万行级、数百依赖的工程里，即使只改一行代码，configuration、资源 link、打包或安装阶段仍会消耗固定时间。

Jugg 的目标不是让 Gradle 本身更快，而是在日常开发场景下减少进入这些固定阶段的次数。下面几节分别说明它在工程信息、源码编译、资源编译和部署四个环节如何做到这一点。
...
代价也很明确：基线本身会过期。一旦 `build.gradle`、依赖或编译参数变化，或本轮改动无法用增量结果证明可信，就必须回到 Gradle 重建基线（见[何时回到 Gradle](#何时回到-gradle)）。

## 工程信息的双源校验

增量编译需要准确的 classpath、source roots、resource roots、applicationId 和 APK 归属。这里的瓶颈不是速度，而是单一信息源的可信度。

仅读 IDE project model 速度快，能立刻拿到模块、source set、运行配置和设备交互信息，但偶尔会漏掉只有 Gradle 执行阶段才能确认的运行时依赖，或高版本 AGP 调整后的输出路径。仅读 Gradle 信息准确，但要等 Gradle 执行后才能刷新。

Jugg 因此同时读取两份信息，并合并为一份项目快照：

| 来源 | 用途 | 局限 |
|---|---|---|
| IDE project model | 快速拿到模块、source set、运行配置和设备交互信息 | 可能缺 Gradle execution phase 才能确认的依赖或产物路径 |
...
源码增量编译的基本思路是复用基线中的 class 和依赖，只把变化源码交给编译器。但“调用编译器”本身在两类语言上都不是直接调用，否则会撞上进程内的类冲突和资源链接的固定耗时。

### Kotlin 编译器的类冲突与 Smart Cast 对齐

直接在 IntelliJ IDEA 进程里调用 Kotlin 编译器会遇到一个时序与依赖冲突：编译器和 IDEA 进程中可能同时存在同包名、但实现不同的类，混用会让编译行为不确定。
...
代价是资源表只增不减：删除资源后，对应 ID 不会立刻从 `resources.arsc` 中消失，要等下一次 Gradle 构建刷新基线。因此 `inclink` 适合 debug 开发，不用于生产构建。
...
release/minify 场景还要额外处理 R8 inline 和被移除的成员；常量引用有独立的分析入口，避免 `const val` 这类编译期内联的改动只更新定义方、漏掉所有内联使用方。代价是分析范围越大，本轮编译的文件越多；当扩散范围过大时，继续增量未必比直接 Gradle 更快。

## 混合部署：产物与设备状态驱动的路径选择

增量运行通常只生成局部产物：DEX、resource overlay、assets、native lib 或需要写回 APK 的文件。直接重装 APK 会丢掉增量带来的速度收益，因此部署阶段要按产物类型和设备状态选择路径。

| 路径 | 适用场景 | 结果 |
|---|---|---|
| Hot reload | 方法体等可在线替换的 class 变化 | 尽量不重启 App |
| Hot fix | class 结构变化、已加载 class 或需要 overlay 恢复 | 通常需要重启 App |
| Reinstall / Gradle fallback | APK 需要完整刷新，或状态不可信 | 重新建立设备与本地基线 |

Jugg 复用 Android Studio Apply Changes 通道下发在线替换和 overlay，同时维护自己的部署历史。这里有一条关键约束：只有整轮部署成功后，staging 产物才提交为新的历史状态；失败轮不能更新全局部署历史，否则下一轮会以错误基线继续增量，造成累积性偏差。

### 运行时与环境对抗

在线替换 class 依赖运行时改写能力，Jugg 通过自带的 JVMTI agent 提供。围绕 agent 有几处针对真实设备环境的防御性设计：

- **加载时序对抗**：Android Studio Apply Changes 首次部署会清理 app 的 startup agents，可能把 Jugg agent 一起删掉。因此 Jugg 把 agent 的推送放在部署之后，并在 App 重启后才检测 JVMTI 是否可用——startup agent 只有进程启动时才会被系统加载。
