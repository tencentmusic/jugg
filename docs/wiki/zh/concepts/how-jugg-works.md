...
常见触发条件包括构建目标变化、工程配置超出当前增量能力、关键基线缺失，以及增量编译或部署无法可靠恢复。具体条件和用户可见结果见[重试、重装与 Gradle 构建](./fallback-and-limits.md)。
...
- [重试、重装与 Gradle 构建](./fallback-and-limits.md)：何时恢复、重装或执行完整构建。
