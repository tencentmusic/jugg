...
这里刻意让引用候选**不要求定义方已经被扫描**。两者解耦后，即使全量扫描尚未完成，编译前也能用已完成的缓存先查出一部分结果。
...
常量索引以本地缓存形式落盘，并按文件指纹复用解析结果：未变化的文件可以直接复用已有指纹，跨同一仓库的多个 worktree 也能共享指纹，减少冷启动时的重复解析。会话内还有一层热点缓存加速点查，但跨轮影响查询始终以落盘缓存为准。
