...
| 比较基线 | 用途 |
|---|---|
| 上一次构建保存的依赖 | 展示相邻两次构建之间新增、更新或删除了哪些库 |
| 最近一次完整 Gradle 构建 | 决定哪些库文件真正需要编译、替换或从设备移除 |
...
| 删除曾增量部署的 class 依赖 | 移除对应的 library DEX |
...
- 删除依赖会移除 Manifest、资源、assets 或 native lib，不能只清理 library DEX。
