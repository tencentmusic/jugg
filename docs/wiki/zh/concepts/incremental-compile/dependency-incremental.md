...
最终进入 APK 的库不一定都出现在编译 classpath。例如，`implementation` 引入的库可以通过 Maven runtime scope 继续带入另一个传递库；应用源码不直接编译引用这个传递库，但它仍会被 D8/R8 处理并进入 APK。Jugg 因此同时读取 Application 与 Dynamic Feature 当前 variant 的编译依赖和运行时依赖，再按实际 library 文件去重。完整构建基线已经记录运行时依赖后，只在运行时依赖图中出现的 AAR/JAR 也会进入依赖变化对比，避免增量部署成功后设备仍运行 APK 中的旧 library DEX。
...
运行时依赖是否参与比较也由完整构建基线统一决定。完整基线中存在运行时依赖数据时，与上一次构建和与完整基线的两个 diff 都比较编译依赖与运行时依赖；完整基线的运行时依赖为空时，两个 diff 都保持原有编译依赖范围。这样即使一次增量读取已经采集到运行时依赖，也不会出现展示 diff 识别变化、实际编译 diff 却忽略变化的分裂结果。

旧版本建立的完整基线没有运行时依赖数据。升级后 Jugg 不会让这份基线失效，也不会把当前项目全部运行时库当成新增；它会继续使用编译依赖范围。之后任一次成功完整 Gradle 构建写入运行时依赖基线，后续增量运行才开始检测 runtime-only 库。这意味着旧基线升级后，runtime-only 变化不会立即生效，但普通用户也不会被统一强制完整构建。
