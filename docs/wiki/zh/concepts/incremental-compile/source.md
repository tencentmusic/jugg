...
删除整个源码文件不会形成新的编译输入；重命名时新路径可以参与编译，但旧路径的删除仍不会生成 class 移除操作。因此不再由新源码产出的旧 class 会继续留在已安装 APK 或增量部署结果中，直接引用、反射和类加载仍可能访问它。需要验证 class 已经不存在时，必须通过完整 Gradle 构建生成新的 APK 基线。
