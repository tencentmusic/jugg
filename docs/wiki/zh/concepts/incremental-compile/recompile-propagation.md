...
一次源码修改可能删除方法、改变字段类型，或给抽象父类和接口新增抽象方法。直接改动的文件可以编译成功，旧 APK 中没有参与本轮编译的调用方或子类却仍按原来的方法签名、字段类型或抽象方法集合运行。如果只编译直接变更的源码，应用运行到相关代码时会抛出 `NoSuchMethodError`、`NoSuchFieldError` 或 `AbstractMethodError`。
