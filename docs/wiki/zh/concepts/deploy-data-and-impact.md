---
title: 部署数据与影响分析
description: 从“只部署被改 class 会漏掉调用方”这一风险出发，说明 Jugg 如何通过新旧结构对比和引用索引扩散补编译、归类各类产物，并交代影响分析的边界与代价。
status: active
tags:
  - concept
  - deploy
...
如果部署阶段只看“本轮生成了哪些文件”，就会漏掉旧 APK 中仍然引用这些变化的调用方，也可能把资源、DEX、Manifest、native lib 用错生效路径。Jugg 因此先做影响分析和产物归类，再决定继续补编译、在线替换、热修复或写回 APK。
...
问题的本质是：编译期只看到“被直接修改的文件”，而运行时崩溃发生在“依赖这些文件的调用方”。增量部署要正确，就必须在下发前把受影响的调用方一并找回来重新编译。Jugg 把这一步拆成两件事——把产物按生效方式分组，以及沿引用关系做防御性扩散补编译。
...
| 产物分组 | 包含什么 | 生效方式 |
|---|---|---|
| 新增 class | 旧索引里不存在的 class | 通常不能只在线替换，需随热修复或重启生效 |
...
这两类补偿是字节码层面的修正。合并结果时有一条优先级：如果某个 class 已经被判定需要源码重编译，就保留源码重编译，不降级为字节码补偿——源码重编译能力更强，反向不成立。
...
这套分析的正确性建立在几个前提之上，也因此有清晰的边界。它为了正确会把受影响调用方都拉进来，扩散范围越大，本轮要编译的文件就越多；当范围过大时，继续增量未必比直接 Gradle 更快。泛型扩散也不是无所不包：它只覆盖子类声明链和对变化类的直接引用方，DEX 擦除后那些没有直接成员引用的间接泛型约束变化，不保证一定命中。

更根本的前提是历史索引必须可信。引用查询基于上一次基线建立的 APK 与部署索引，一旦基线过期或被旁路改动，分析结果也会随之失真，这时需要回到 Gradle 重建基线。最后要分清职责：产物分组只决定“怎么发”，不决定“一定成功”——能否在设备上真正生效，仍取决于设备状态和运行时兼容性，见[部署策略](./deploy-strategy.md)与[部署状态与恢复](./deploy-state-recover.md)。
