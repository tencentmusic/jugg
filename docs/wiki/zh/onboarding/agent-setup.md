...
云开发机配置是可选项。它适合本地机器编译慢、磁盘空间紧张，或团队已经准备好远程构建资源的场景。配置完成后，本地 Android Studio 仍然负责入口和部署，Gradle 构建可以放到远端机器执行。

Jugg 会记住最后一次成功的远程编译参数。新工程首次打开时会带入这套配置，后续如果在某个工程里修改，不会自动同步到其他已打开工程。

## 申请云开发机

优先使用团队提供的 Jugg 云编译通用模板。这类模板通常已经预装 Java、Android SDK、Git 等基础环境，Gradle home 指向大容量磁盘，申请完成后可以直接用于编译。

申请时建议选择较高配置，例如 32 核、64G 内存、500G 硬盘。硬盘后续通常可以扩容；CPU 和内存扩容可能需要重启，并且要注意 `/data` 之外的数据是否会丢失。
...
在 Jugg Run Configuration 中打开远程编译配置，然后选择同步模式。同步模式决定本地源码如何上传到云开发机，以及远端构建产物如何拉回本地。
...
Windows 不支持 rsync，必须使用 iFT。已经配置 iFT 的 Mac 或 Linux 也可以继续使用 `rsync_simple`。

## 多工程同步

如果你的工作区包含多个互相依赖的工程，可以开启多工程开发模式。开启后，每次编译前会同步 `Local to remote sync path` 下的所有工程。
...
| `SSH password/key(optional)` | 登录密码或 SSH key。已配置 SSH key 时建议留空，让 Jugg 自动搜索 `.ssh` 目录 |
| `SSH host` | 云开发机 IP 或域名 |
| `SSH port` | SSH 端口，腾讯云服务器常见端口为 `36000` |

密码留空时，如果无法通过 key 登录，Jugg 会在运行时弹窗要求输入。
...
如果远端已经有固定同步目录，并且不是默认路径，可以主动填写，避免重复同步。
