...
彻底隐藏 `layout_dump`，LLM 完全不感知它的存在。

## 方向

采用 **Method A**：`figma_layout_verify` 内部自动调用 `layout_dump`，LLM 不需要传 `androidJsonPath`。
...
LLM 调用 `figma_layout_verify` 时无需提前调用 `layout_dump`，工具能正常返回验证结果。
