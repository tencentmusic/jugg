...
collector 通过 `dependsOn` 绑定本轮 `request.json` 已声明的 local external tasks；included build 继续沿用现有子 collector 汇总关系。不得额外依赖 APK owner strip、非本轮 selected module merge 或其它 native task。生产侧派生命令始终同时请求 external task(s) 与 collector，没有只请求 collector 的产品入口。
...
| L3 等价 Flow | `ReadProjectInfoGradle9CompatTest#generatedScript_shouldBuildBeforeStrippingSelectedNativeOutput` | 真实 AGP 8.7.2 + NDK 27 + CMake：修改 C++ 后以测试方式只请求 collector，验证 `dependsOn` 会先执行选中的 app merge、但不执行 app strip；Jugg stripped 输出与 AGP `:app:stripDebugDebugSymbols` 输出 SHA-256 完全一致。该调用只用于验证依赖编排，不代表产品入口 |
