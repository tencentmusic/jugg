...
15. **只有"间接继承"形态才产生 supertype 诊断族，实现后用真实 `android.jar` 复核过。** 用 `android-34/android.jar`（确认无 `WifiManager$SoftApCallback`）+ 自造 `rom_framework.jar`（完整 `WifiManager` 含该嵌套类）+ `rom_sdk.jar`（`com.rom.RomSoftApCallback extends WifiManager.SoftApCallback`），Kotlin 1.9.22 实测：
    - Kotlin 源码里**直接书写** `WifiManager.SoftApCallback`（`import` / FQN / 嵌套三种写法都试过）→ 一律是 `unresolved reference: SoftApCallback`，**不命中**本方案的诊断族。
    - Kotlin 源码只是**引用**（构造、返回类型）`RomSoftApCallback` → 两种 classpath 顺序都编译成功，不需要解析 supertype 链。
    - Kotlin 源码**继承** `RomSoftApCallback`（现场形态）→ 才逐字复现 report 的两条诊断：`unresolved supertypes: android.net.wifi.WifiManager.SoftApCallback` 与 `cannot access '...' which is a supertype of '...'`。
    - 同一组 jar、同一顺序下 javac 继承同一个类**两种顺序都成功**，再次确认事实 13 与"Java 分支不实现"。
    结论：触发面比放宽后的匹配规则看起来更窄，真正会命中的是"源文件继承了某个 jar 里的类，而那个类的 supertype 被前置 stub 挡住"。第二条诊断带 `file:line`，会进 `directErrorFiles`，按文件记忆能绑到正确文件（第一条不带位置，靠收窄条件排除）。
...
测试价值：诊断判定与"把第一个 `android.jar` 移到末尾"是独立、稳定、可被真实破坏的契约；通过门禁，落 L1，behavior owner 为新建 helper。现有 `BaseCompileContextModuleDependenciesTest` 覆盖的是 included-build R.jar 顺序，不是 overlay，不要塞进该类。

重试编排本身（"失败后换序再编一次并恢复"）按 §3 分层属于 L2。原方案以"薄状态、不加 mock seam"为由不测，实现后修正：该行为可以用真实编译器覆盖，不需要任何测试专用 seam，因此在 `android_demo_project` 里落地了 fixture 场景，owner 复用既有的 `SourceCompileTest`（同类已有 `juggAptRetry_*` 两个重试编排用例）。

fixture 构成（`app/romlibs/`，`compileOnly`，运行时由设备提供，故不入 dex）：

| 文件 | 内容 | 作用 |
|---|---|---|
| `rom_framework.jar` | `android.net.wifi.WifiManager` + 嵌套 `SoftApCallback` | 扮演 ROM 完整 framework，被前置的公开 `android.jar` 遮蔽 |
| `rom_sdk.jar` | `com.rom.RomSoftApCallback extends WifiManager.SoftApCallback` | 提供"supertype 在 framework jar 里"的二进制类 |
| `testcase/romhiddenapi/RomHiddenApiChild.kt` | 继承 `RomSoftApCallback` | 迫使 Kotlin 解析整条 supertype 链，命中诊断 |

`src/` 下保留两个 `.java` 源用于重新生成 jar，生成命令写在 `app/build.gradle` 的注释里。`compileOnly` 同步到 5 个 `build.gradle.kotlin*` 变体，版本切换后仍生效。

选这套 fixture 的前提已实测：加入后 `:app:compileDebugKotlin` 与 `:app:assembleDebug` 均成功（`dexBuilderDebug` 无缺类报错），即 Gradle 侧不受影响——这正是现场"Gradle 能编、Jugg 编不过"的不对称性，也是该场景能长期留在 demo 工程里的原因。
...
| L2 | `SourceCompileTest.romHiddenApi_shouldRecoverWhenSdkAndroidJarShadowsFrameworkJar` | 真实编译 `RomHiddenApiChild.kt`，`android.jar` 在默认首位 | 首次失败后换序重试，最终 `isAllSuccess` |
| L2 | `SourceCompileTest.kotlinAndJavaCompile` / `kotlinAndJavaCoreRefrenceCompile` | 普通增量编译（无 supertype 诊断） | 仍成功；默认 classpath 不被本功能改写 |
| L2 | `SourceCompileTest.juggAptRetry_*` | 另一条重试链路与本功能共用重试预算 | 仍成功，预算未被抢占 |
| 失败证据 | report `06f3d8fa` + §2.2 本地实验 + 上表 L2 用例 | 首次诊断、`android.jar` 位于首位、换序后编译成功 | 已取得 |

失败证据（实现后补取，作为 L2 的红灯基线）：把 `KotlinCompilerInvoker` 与 `AndroidJarClasspathRetry` 回退到修复前，`romHiddenApi_*` 失败于

```text
unresolved supertypes: android.net.wifi.WifiManager.SoftApCallback
cannot access 'android.net.wifi.WifiManager.SoftApCallback' which is a supertype of
    'com.sickworm.jugg.demo.testcase.romhiddenapi.RomHiddenApiChild'
```

与 report 逐字一致；恢复修复后通过。
...
./gradlew :main:test \
  ...
  --tests "com.sickworm.intellij.jugg.compiler.SourceCompileTest" \
  :idea:compileKotlin
```
...
- [ ] L1 测试与整个 `SourceCompileTest`（含新增 `romHiddenApi_*`）通过；知识库已同步。
- [ ] demo 工程加入 fixture 后 `:app:assembleDebug` 仍成功，其他依赖该产物的测试不受影响。
