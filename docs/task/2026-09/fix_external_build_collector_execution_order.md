...
- `request.json` 是 `ExternalBuildTaskRunner` 为单次 invocation 生成的临时执行与收集清单，记录 invocation id 及本轮 module/variant/task/type；它不是持久化项目配置，也不表示一个只读 metadata 的独立请求。
- 生产代码中 `deriveExternalBuildCommand()` 要求 external task path 非空，并将这些 task 与 `juggCollectExternalBuildInfo` 放在同一条 Gradle 命令中；目前 Jugg 没有只请求 collector 的产品场景。
- collector 对 local request task 建立 `dependsOn`，用来保证即使调度器改变命令列表的执行顺序，收集也只会在本轮产物完成后开始。
- 测试可以单独请求 collector，用于直接验证上述依赖编排；这是测试手段，不是需要维持的产品契约。
- 旧的 `TaskState.executed` 顺序检查已移除。`ExternalBuildTaskRunner.readUpdates()` 仍校验有效结果、invocation id 和 request/update key 完全一致，这是结果完整性门禁，不是任务顺序检查。

验证覆盖：

- 验证已注册和延迟注册的 module task 均进入 collector 的 task dependencies。
- 在真实 AGP + NDK fixture 中修改 C++ 源码后，以测试方式只请求 collector，确认依赖编排会先执行 native build/merge，再生成与当前 AGP strip 结果逐字节一致的输出。
