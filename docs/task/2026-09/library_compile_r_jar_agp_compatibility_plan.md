...
- 后续评审修正：synthetic androidTest 虽建模为 `Library`，但必须保留自身 test variant aggregate R 的优先级，避免 owner 主 variant R 遮蔽 test resource。
...
| synthetic androidTest（`isAndroidTestModule`） | own test variant aggregate `rFilePath` |
...
- androidTest 自身 test variant aggregate R 位于 owner module 主 variant R 之前。
...
| L1 | `#androidTest module uses own aggregate R before owner module R` | self-targeting Library androidTest 的 test R 与 owner 主 R 同 namespace | owner 主 R 先命中，test resource 字段缺失 | test variant aggregate R 先命中，真实 javac 读取 test 字段 |
...
- synthetic androidTest 使用自身 test variant aggregate R，并保持在 owner module 主 variant R 之前。
