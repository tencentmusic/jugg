...
```
         ┌──────────────────────────────┐
    L3   │  手测矩阵（§10.4）+ FlowTest  │  真机 / demo 工程端到端
         ├──────────────────────────────┤
    L2   │  idea 协作测试（§10.3）      │  IDE API + Mockito 接口
         ├──────────────────────────────┤
    L1   │  main 域内测试（§10.2）      │  解析器 / 派生 / 序列化 / 状态机
         └──────────────────────────────┘
```

**口径**：每条对外主链路须有 L3（或 §10.4 手测矩阵）；§10.2 为 L1 域内测试（可多、可快），**不**表示应为编排类堆单文件 Mockito 单测。TDD 前置条件见 `AGENTS.md` 与 06_testing §8。

### 10.2 main 模块域内测试（L1，必须先写）
...
### 10.3 idea 模块协作测试（L2，IDE API 依赖）
