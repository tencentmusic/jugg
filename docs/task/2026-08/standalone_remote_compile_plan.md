...
让 standalone Runtime 可以执行当前已选的 remote compile profile。`init`、`compile`、`deploy` 与 `gradle-build` 应复用 IDEA 已有的远程编译链路，而不是要求用户切换到 local profile 或重建 profile。
...
2. `compile`、`deploy`、`gradle-build` 将该 profile 转成原有 `JuggGradleCompileOptions`，由 `JuggCompilerHelper` 选择远程 client。
...
- standalone 只能使用 profile 中已有的密码/密钥路径，或本机已发现的 `~/.ssh` 密钥。
- 需要补录 SSH 密码、密钥口令、iFT 用户名或 iFT 密码时，立即以明确的结构化失败结束；不得尝试桌面对话框、stdin 提示或将秘密写入日志/JSON。
...
| 编译/产物 | standalone 分发仍包含远程 client 与 JSch | `./gradlew :cmd_line:installDist`，检查运行时 classpath/启动 |
| L3 手工 | 已配置 remote profile 的 `gradle-build` | 在具备 SSH、同步工具和远端工程的受控工程上运行，确认远端 Gradle、APK/classpath 回拉及 `get-compile-status=success` |
...
2. L3 使用哪个可访问的远端环境，以及该 profile 采用 rsync 还是 iFT；这决定最终验证命令，但不改变本次核心代码设计。
3. `deploy` 的设备仍连接在 standalone 所在机器，远端仅负责构建；是否符合目标使用方式。
