...
`JuggCliAutoUpdater`、`McpFetchCleaner` 等全局或后台任务仍需确定唯一执行 owner，或使用全局 named lock 避免 IDEA 与 daemon 重复写入。
