...
- 仓库已使用法务提供的完整 `LICENSE`，并提供 `THIRD_PARTY_NOTICES.md`、104 行 `third_party/components.csv`、许可证文本、对应源码、第三方修改 changelog、集成边界说明和 SPDX 2.3 SBOM；插件携带相同 `LICENSE`、精确公开源码 revision 和校验值，不重复打包源码 payload。
...
- `:idea:buildPlugin` 会把根目录 `LICENSE`、`THIRD_PARTY_NOTICES.md`、排除 `sources` payload 的 `third_party`、精确源码 revision 和源码校验值复制到插件根目录。
- `:idea:verifyThirdPartyCompliance` 校验根目录 LICENSE 与发行包副本、NOTICE、104 行组件清单、第三方修改 changelog、集成边界说明、许可证、公开源码定位、OpenJDK 修改 patch、仓库源码 SHA-256、CI 源码 Git 状态、插件内无源码 payload 和 104 个 package 的 SPDX 2.3 SBOM；缺失或不匹配时构建失败。
- 法务关注的地址空间边界已按实际实现记录在 `third_party/INTEGRATION.md`：`rsync`、`sshpass` 为独立进程；Checker Qual 和 JavaBeans Activation Framework 分别选择 MIT、CDDL-1.1；OpenJDK JVMTI header 使用 GPL-2.0 WITH Classpath Exception；Trove4J、juniversalchardet 保持独立 JAR 动态加载。该技术事实仍需法务确认其法律口径。
...
- 根目录已有法务提供的完整 `LICENSE` 和 `THIRD_PARTY_NOTICES.md`，发行包内包含相同 LICENSE、等价 NOTICE、104 行机器清单、许可证、公开源码 revision 与校验值、第三方修改 changelog、集成边界说明和 SPDX 2.3 SBOM；源码 payload 保留在公开 Git revision。
