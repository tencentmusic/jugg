...
- First raw Gradle and stop blocks include a plain key-value `Jugg status` summary, including `enabledAndroidTest`, so the agent can reuse that context without running `jugg status` again.
