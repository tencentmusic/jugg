...
**期望调用序列**：
1. 通过 jugg-android-dev-loop 执行 `ui_find(target={text: "Unique MCP Target"})` 或等价的 view_locate 调用
2. 返回元素位置与大小（bounds）
...
**期望输出行为**：
- 命令返回成功，包含 bounds 信息（x, y, width, height）
- LLM 正确报告元素位置
...
| 4 | 参数正确但报告了不必要的信息 |
| 3 | 调用了正确命令，但对结果解读有偏差 |
| 2 | 使用了 layout_dump 代替定位命令，但结论正确 |
| 1 | 关键参数错误或使用废弃命令 |
| 0 | 未调用任何命令 |
...
| 4 | 参数正确但结论表述不够准确 |
| 3 | 命令调用正确但参数使用了 text 代替 resourceId |
| 2 | 使用错误命令但结论正确 |
| 1 | 方向性错误 |
| 0 | 未调用命令 |
...
**期望调用序列**：
1. 通过 jugg-android-dev-loop 执行 `eval_view(target={resourceId: "tv_mcp_style_title"}, expressions=["getText().toString()"])`
2. 返回文本值 `"Style Title"`
