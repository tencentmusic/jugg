...
1. Default command: `deploy` (compile + deploy, waits for completion).
2. If user asks compile-only or no device is available: `compile`.
3. If incremental path is likely insufficient: `gradle-build`.

## Async Compile Rule (Mandatory)

The CLI handles polling internally — `deploy` and `gradle-build` block until completion and print progress to stderr. No manual `get_compile_status` polling needed.
...
3. If still failing, use `gradle-build` (heavy).
   - `gradle-build` produces the compiled artifact only (no deploy). After it completes, run `deploy` to push the artifact to device (compile phase will be skipped because output is already up-to-date).
   - On final failure, inspect `${projectDir}/build/jugg/log/compile_latest.log`.
4. Retry `deploy` once after `gradle-build`.
5. If still broken, inspect `${projectDir}/build/jugg/log/compile_latest.log`.
6. Only when install-state corruption or signature conflict is likely, run `reinstall` as a post-step.
7. If still unclear, stop and confirm with user.
8. Remote troubleshooting (`ssh-info --reason <reason>`) requires explicit user consent.

Important:

- Do not place `reinstall` before `gradle-build`.
- `reinstall` is conditional recovery, not a general retry.

Special case:

- `MCP_NO_DEVICE` / `No device`: stop and ask user to connect/start device, or switch to `compile`.
