...
Use when executing runtime interaction or UI verification. `jugg screenshot` / `jugg record-start` / `jugg record-stop` are OUT OF SCOPE — do NOT call them when this Skill is active.
...
**Steps**:
1. `jugg activity-stack` → read current activity
2. Output mandatory gate line: `PageGate: stack=<ActivityName> target=<targetPage> match=<yes|no>`
3. On `match=no`: `jugg restart [--tap "tap:text=<step>" ...]` with `navigationSeq` → repeat from step 1
...
1. `jugg view-locate --text <text>` to get element bounds
...
### `jugg view-inspect [--text <text>] [--id <resourceId>] [--desc <contentDesc>] [--class <className>] <expr>...`
...
2. `jugg view-locate --text <element>` per element → calculate spacing from bounds
3. `jugg view-inspect` for unsupported properties
