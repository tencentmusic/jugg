...
3. If `force_gradle_compile` returns `isFinal=false`, poll `get_compile_status(jobId)` to terminal state
4. `force_gradle_compile` final status = success -> retry `compile_and_deploy`
5. Still fails -> try `clean_reinstall_apk`
6. `clean_reinstall_apk` fails -> stop and report to user with full diagnosis
