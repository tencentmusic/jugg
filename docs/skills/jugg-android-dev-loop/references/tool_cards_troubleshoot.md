...
- Default path is CLI-only (`jugg <subcommand>`).
...
1. Run `jugg crash-report` to identify likely cause.
2. If `jugg crash-report` is unavailable or returns empty/unusable result, fallback to `adb logcat` for current session.
...
4. Route by result:
   ...
   - `hasCrash=false` + `isProcessAlive=false`: run `jugg restart` and rerun Target Page Context Gate; if recurring, stop and ask user.
   - `hasCrash=false` + `isProcessAlive=true`: page state issue — refresh `jugg layout-dump` and retry navigation within retry budget.
