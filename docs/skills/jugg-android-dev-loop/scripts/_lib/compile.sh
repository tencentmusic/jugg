#!/usr/bin/env bash
# cmd/compile.sh — compile modified sources without deploying
...
cmd_compile() {
  ...
  response=$(_jugg_raw_call "$port" "compile_only" "$params")
...
")
  ...
  # compile_only may return isFinal=false; poll to termination
