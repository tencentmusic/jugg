...
public abstract class JuggAbstractDeployTask implements LaunchTask {
    ...
    private final JuggDeployData myJuggDeployData;

    public static final Logger LOG = Logger.getInstance(AbstractDeployTask.class);

    public JuggAbstractDeployTask(@NotNull Project project, @NotNull Map<String, List<File>> packages, boolean fallback,
                              Computable<String> installPathProvider, JuggDeployData juggDeployData) {
        myProject = project;
        myPackages = packages;
        myFallback = fallback;
        myInstallPathProvider = installPathProvider;
        mySubTaskDetails = new ArrayList<>();
        myJuggDeployData = juggDeployData;
    }
...
//        LogWrapper logger = new LogWrapper(LOG);
        ...
        for (Map.Entry<String, List<File>> entry : myPackages.entrySet()) {
            ...
            try {
                JuggDeployer.Result result = perform(device, deployer, applicationId, apkFiles, myJuggDeployData);
    ...
    abstract protected JuggDeployer.Result perform(
            IDevice device, JuggDeployer deployer, String applicationId, List<File> files, JuggDeployData data) throws DeployerException;
